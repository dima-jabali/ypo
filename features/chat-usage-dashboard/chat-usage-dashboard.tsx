...
function DefaultHeader({
	...
	column: Column<ChatUsageData, string | number>;
}) {
...
function DownloadAllConversationsButton({
	userId,
}: {
		...
		<section className="@container flex flex-col gap-14 h-full w-full p-6 simple-scrollbar">
			...
			<div className="grid grid-cols-1 gap-4 @xl:grid-cols-2 @4xl:grid-cols-3 @7xl:grid-cols-4">
				{Object.entries(stats).map(([key, value]) => (
					<Card
						className="@container/card bg-aside border-border-smooth/50"
						key={key}
					>
						<CardHeader>
					...
					<Table className="overflow-hidden rounded-lg">
						<TableHeader>
							{table.getHeaderGroups().map((headerGroup) => (
								<TableRow
									className="bg-aside hover:bg-aside font-bold"
									key={headerGroup.id}
								>
