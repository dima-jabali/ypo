...
export const SEARCH_FOR_MATCHES_ON_ALL_BLOCKS =
	"search-for-matches-on-all-blocks";

export const handleCopyCodeToClipboard = async (
...
export const getConnectionInfo = (query: SimilarQuery["query"]) => {
	...
	switch (query.entity_type) {
		...
		case QueryEntityType.MODE_DEFINITION:
			connectionInfo.name =
				query.mode_definition.mode_data_connection?.name || "";
			break;
...
export function getDbInfoJSX(
	...
	switch (query.connection_type) {
		...
		default:
			console.error(
				"Query database type not supported:",
