...
import { BrieflyKeepSidebarOpen } from "#/components/briefly-keep-sidebar-open";
...
export function SimilarQueriesOfBlock() {
	const [filterBySimilarQueries, setFilterBySimilarQueries] =
	...
	const sortedAndFilteredBlocks = useMemo(() => {
		...
		for (const similarQuery of similarQueriesToShow.similarQueries) {
			if (
				...
				SIMILAR_QUERIES_FILTER[filterBySimilarQueries] ===
					similarQuery.query.entity_type
			) {
		...
		return result.sort(
			sortBy.sortBy === SortBy.Overlap ? overlapSortFn : relevanceSortFn,
		);
	...
	return (
		<>
			<BrieflyKeepSidebarOpen
				value={isSortPopoverOpen || isFilterPopoverOpen}
			/>
			...
			<header className="flex items-center justify-between gap-2 border-b py-1 border-border-smooth align-middle text-base font-bold">
				...
				>
					<ChevronLeftIcon className="size-3 stroke-primary stroke-1" />

					<p>All blocks</p>
				</button>

				<div className="flex h-full w-full items-center justify-end gap-1">
					<Popover onOpenChange={setIsSortPopoverOpen} open={isSortPopoverOpen}>
						<PopoverTrigger
							className="flex aspect-square h-full items-center justify-center rounded-sm button-hover data-[state=open]:bg-button-active"
							title="Sort by"
						>
							<ChevronsUpDownIcon className="size-5 stroke-primary" />
						</PopoverTrigger>

						<PopoverContent
							className="flex flex-col gap-1 p-1 text-sm"
							align="center"
							sideOffset={5}
							side="bottom"
						>
								...
								return (
									...
									>
										...
										{sortBy.order === SortOrder.ASC ? (
											<ChevronUpIcon
												className="size-3 data-[should-show=false]:invisible text-primary"
					...
					<button
						...
						title="Expand all"
					>
						<ExpandAllIcon className="size-5 fill-primary" />
					</button>
					...
					<button
						...
						title="Collapse all"
					>
						<CollapseAllIcon className="size-5 fill-primary" />
					</button>
					...
					>
						<DropdownMenuTrigger className="h-full flex items-center gap-2 rounded-sm bg-button-hover px-2 py-1 text-xs font-bold tracking-wider active:bg-button-active! whitespace-nowrap">
							<span className="min-w-0 truncate max-w-6 xl:max-w-16">
								{filterBySimilarQueries}
							</span>
...
function SearchForMatches() {
	const searchTextForSimilarQueries =
		generalContextStore.use.searchTextForSimilarQueries();
	...
	return (
		<section className="relative flex items-center justify-center p-2">
			<div className="flex w-full shrink-0 items-center justify-start gap-1 rounded-full border border-border-smooth p-1 focus-within:ring-1 focus-within:ring-accent/40">
				...
				<input
					className="h-full flex-1 shrink-0 overflow-clip text-ellipsis bg-transparent text-sm outline-hidden"
