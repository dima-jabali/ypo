	...
	return (
		<>
			<Popover onOpenChange={setIsOpen} open={isOpen}>
				<BrieflyKeepSidebarOpen value={isOpen} />

				<PopoverTrigger className="flex items-center justify-between gap-2 rounded-sm bg-primary/5 button-hover p-2 text-xs font-bold tracking-wider data-[state=open]:bg-button-active">
					{filter}
				...
				<PopoverContent
					className="flex flex-col w-[unset]"
					side="bottom"
					align="end"
				>
					{FILTERS.map((filterString) => {
						...
						return (
							<button
								className="button-hover py-1 px-2 rounded flex w-full items-center justify-between"
			...
			>
				<div className="flex flex-col gap-2 py-2">{filteredBlocks[filter]}</div>
			</div>
		</>
	);
}
