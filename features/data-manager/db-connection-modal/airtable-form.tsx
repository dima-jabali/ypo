...
export const AirtableForm: React.FC<{
	...
	return (
		<>
			...
			<DialogDescription>
				To add a Airtable connection, you need to to authorize BetterBrain to
				connect to your desired Airtable&apos;s workspace.
			</DialogDescription>

			<section
				title="How to create an auth token for Airtable"
				...
				<details className="text-sm">
					...
					<div className="flex flex-col gap-4 pl-4 mt-2 [&_span]:font-bold tabular-nums">
						<p>
							...
							, click the &quot;Create new token&quot; button to create a new
							personal access token.
						</p>

						<p>
							<span>2. </span>
							Give your token a unique name. This name will be visible in record
							revision history.
						</p>
						...
						<p>
							...
							to grant to your token. This controls what API endpoints the token
							will be able to use.
						</p>

						<p>
							<span>4. </span>
							Click &quot;add a base&quot; to grant the token access to a base
							or workspace.
						</p>

						<p>
							<span>5. </span>
							You can grant access to any combination and number of bases and
							workspaces. You can also grant access to all workspaces and bases
							under your account. Keep in mind that the token will only be able
							to read and write data within the bases and workspaces that have
							been assigned to it.
						</p>

						<p>
							<span>6. </span>
							Once your token is created, we will only show it to you once, so
							we encourage you to copy it to your clipboard and store it
							somewhere safe. While you will be able to manage it in{" "}
							<a
								className="text-link hover:underline hover:text-link-hover visited:text-link-visited"
								href="https://airtable.com/create/tokens"
								target="_blank"
			...
			<form className="my-6 flex flex-col gap-4" ref={formRef}>
				...
				<fieldset className="flex items-center gap-4">
					<label
						className="whitespace-nowrap font-bold"
						htmlFor={AIRTABLE_PAT_INPUT_ID}
					>
						Airtable Personal Access Token:
					</label>

					<Input
						name={AIRTABLE_PAT_INPUT_ID}
						id={AIRTABLE_PAT_INPUT_ID}
						type="password"
						required
					/>
				</fieldset>
