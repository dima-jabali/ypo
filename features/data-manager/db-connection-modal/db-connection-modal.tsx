...
const CONNECT_TO_SLACKS_WORKSPACE_URL = import.meta.env
	.VITE_PUBLIC_CONNECT_TO_SLACKS_WORKSPACE_URL;

if (!CONNECT_TO_SLACKS_WORKSPACE_URL) {
	throw new Error("Missing VITE_PUBLIC_CONNECT_TO_SLACKS_WORKSPACE_URL");
}
...
export function DBConnectionModalContent({
	...
	async function handleConnectToSlackWorkspace() {
		...
		if (!CONNECT_TO_SLACKS_WORKSPACE_URL) {
			console.error('Missing "VITE_PUBLIC_CONNECT_TO_SLACKS_WORKSPACE_URL"');

			toast({
				description: "This should never happen! Please, contact support!",
				title: "Missing VITE_PUBLIC_CONNECT_TO_SLACKS_WORKSPACE_URL",
		...
		<>
			...
			<>
				...
				>
					<DialogContent
						className={cn(
							"flex max-h-[90vh] flex-col border-border-smooth bg-popover text-primary p-9 pb-6 simple-scrollbar",
							className,
						)}
			...
			<Dialog onOpenChange={setIsErrorDialogOpen} open={isErrorDialogOpen}>
				<DialogContent className="flex flex-col overflow-auto border-border-smooth p-9 pb-6">
					<DialogTitle>
						Error connecting to&nbsp;
