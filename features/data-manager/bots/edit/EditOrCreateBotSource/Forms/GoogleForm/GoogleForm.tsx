...
import {
	...
	type GoogleDriveDatabaseConnectionId,
...
export function GoogleForm({
	...
	const googleDriveConnectionId =
		organizationFilesStore.use.googleDriveConnectionId();
	...
	async function handleCreateOrEditGoogleDriveBotSource() {
		...
		if (!form) {
			console.error(
				"Form not found when trying to create Google Drive bot source.",
			);
			...
			if (action === BotSourceFormAction.Create) {
				const newBotSourceInfo: CreateGoogleDriveBotSourceRequest = {
					google_drive_folder_ids: [
						...selectedFolders.values().map((folder) => folder.id),
					],
