	...
	const allAllowedSlackChannels: SlackChannelWithName[] = useMemo(
		() =>
			botDatabaseConnections
				.find(
					(db) =>
						isSlackBotSource(source) && db.id === source.slack_connection_id,
				)
				?.channels.filter(isIndexableSlackChannel) ??
			DEFAULT_ALL_ALLOWED_SLACK_CHANNELS,
