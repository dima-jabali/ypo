...
type FormDataEntries = {
	[Key in keyof Omit<
		...
		"organizationId" | "start_urls"
	>]: string;
};

export const CreateWebCrawlDialog: React.FC<Props> = ({
	setSelectedWebCrawls,
}) => {
	...
	const handleSendForm = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		const args: CreateWebCrawlRequest = {
			...
			dynamic_content_wait_seconds:
				Number(data.dynamic_content_wait_seconds) || 0.01,
				...
				>
							...
							<InfoPopover
								text={
									<p>
										...
										<br />
										<br />
										By default, the Actor will also crawl sub-pages of these
										URLs. For example, for start URL
										<i className="font-mono"> https://example.com/blog</i>, it
										will crawl also{" "}
										<i className="font-mono">
											https://example.com/blog/post or
											https://example.com/blog/article
										</i>
										. The Include URLs (globs) option overrides this automation
										behavior.
									</p>
								}
							/>
						...
						<section className="flex flex-col gap-2 rounded-lg border border-border-smooth  p-2">
							{startURLsRef.current.map((startURL, startURLIndex) => (
								<div className="flex items-center gap-2" key={Math.random()}>
									<Input
										onChange={(e) =>
											handleUpdateStartURL(startURLIndex, e.target.value)
										}
					...
					<details className="rounded-lg border border-transparent p-2 open:border-border-smooth ">
						...
						<section className="mt-8 flex flex-col gap-4 p-2">
									...
									<InfoPopover
										text={
											<p className="tabular-nums">
												The maximum number of links starting from the start URL
												that the crawler will recursively follow. The start URLs
												have depth 0, the pages linked directly from the start
												URLs have depth 1, and so on.
												<br />
												<br />
												This setting is useful to prevent accidental crawler
												runaway. By setting it to 0, the Actor will only crawl
												the Start URLs.
											</p>
										}
									/>
							...
							<label
								className="flex items-center gap-4"
								htmlFor="dynamic_content_wait_seconds"
							>
								<p className="flex items-center gap-1 whitespace-nowrap text-sm font-bold">
									...
									<InfoPopover
										text={
											<p className="tabular-nums">
												The maximum time to wait for dynamic page content to
												load. By default, it is 10 seconds. The crawler will
												continue either if this time elapses, or if it detects
												the network became idle as there are no more requests
												for additional resources.
												<br />
												<br />
												Note that this setting is ignored for the raw HTTP
												client, because it doesn&apos;t execute JavaScript or
												loads any dynamic resources.
											</p>
										}
									/>
								</p>
							...
							>
								<p className="flex items-center gap-1 whitespace-nowrap text-sm font-bold">
									...
									<InfoPopover
										text={
											<p className="tabular-nums">
												The initial number of web browsers or HTTP clients
												running in parallel. The system scales the concurrency
												up and down based on the current CPU and memory load. If
												the value is set to 0 (default), the Actor uses the
												default setting for the specific crawler type.
												<br />
												<br />
												Note that if you set this value too high, the Actor will
												run out of memory and crash. If too low, it will be slow
												at start before it scales the concurrency up.
											</p>
										}
									/>
								</p>
							...
							<label
								className="flex items-center gap-4"
								htmlFor="expand_clickable_elements"
							>
								<p className="flex items-center gap-1 whitespace-nowrap text-sm font-bold">
									...
									<InfoPopover
										text={
											<p className="tabular-nums">
												...
												at the domains of the provided Start URLs and enqueue
												matching URLs similarly as the links found on crawled
												pages. You can also reference a sitemap.xml file
												directly by adding it as another Start URL (e.g.{" "}
												<i className="font-mono">
												...
												<br />
												<br />
												This feature makes the crawling more robust on websites
												that support Sitemaps, as it includes pages that might
												be not reachable from Start URLs. Note that if a page is
												found via a Sitemap, it will have depth 1.
											</p>
										}
									/>
								</p>
							...
							<section className="flex flex-col gap-1">
								<p className="flex items-center gap-1 whitespace-nowrap text-sm font-bold">
									...
									<InfoPopover
										text={
											<p className="tabular-nums">
												Glob patterns matching URLs of pages that will be
												included in crawling.
												<br />
												<br />
												Setting this option will disable the default Start URLs
												based scoping and will allow you to customize the
												crawling scope yourself. Note that this affects only
												links found on pages, but not Start URLs - if you want
												to crawl a page, make sure to specify its URL in the
												Start URLs field.
												<br />
												<br />
												For example,{" "}
												...
												&nbsp;&nbsp; lets the crawler to access all URLs
												starting with{" "}
												<i className="font-mono">https://store.example.com/</i>{" "}
												or{" "}
												<i className="font-mono">https://docs.example.com/</i>,
												and{" "}
												<i className="font-mono">
													{"https://example.com/**/*\\?*foo=*"}
												</i>
												&nbsp; allows the crawler to access all URLs that
												contain foo query parameter with any value.
												<br />
												<br />
								...
								<section className="flex flex-col gap-2 rounded-lg border border-border-smooth  p-2">
									{includeURLGlobsRef.current.map(
										(includeURLGlob, includeURLGlobIndex) => (
											<div
											...
											>
												...
												<Button
													onClick={() =>
														handleRemoveIncludeURLGlob(includeURLGlobIndex)
													}
													variant={ButtonVariant.DESTRUCTIVE}
							...
							<section className="flex flex-col gap-1">
								<p className="flex items-center gap-1 whitespace-nowrap text-sm font-bold">
									...
									<InfoPopover
										text={
											<p className="tabular-nums">
												Glob patterns matching URLs of pages that will be
												excluded from crawling. Note that this affects only
												links found on pages, but not Start URLs, which are
												always crawled.
												<br />
												<br />
												For example,{" "}
												...
												&nbsp; excludes all URLs that contain foo query
								...
								<section className="flex flex-col gap-2 rounded-lg border border-border-smooth  p-2">
									{excludeURLGlobsRef.current.map(
										(excludeURLGlob, excludeURLGlobIndex) => (
											<div
												className="flex items-center gap-2"
												key={Math.random()}
											>
												...
												<Button
													onClick={() =>
														handleRemoveExcludeURLGlob(excludeURLGlobIndex)
													}
													variant={ButtonVariant.DESTRUCTIVE}
