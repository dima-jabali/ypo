	...
	return (
		<table className="w-full border-collapse border border-border-smooth">
			<thead className="bg-transparent">
				<tr>
					<th className="border border-border-smooth py-1 text-primary">
						Name
					</th>
			...
			<tbody>
				{allBotsOfConnection.map((bot, index) => (
					<tr
						...
						key={index}
					>
						<td className="border border-border-smooth px-2 py-1">
							{bot.name}
						</td>
						...
						<td className="flex-wrap border border-border-smooth px-2 py-1">
							<div className="flex border-collapse flex-wrap gap-2">
								{bot.sources.map((source) => (
									<span
										className="rounded-sm bg-slate-600 text-white px-2 py-1"
										key={source.id}
									>
										{source.name}
									</span>
