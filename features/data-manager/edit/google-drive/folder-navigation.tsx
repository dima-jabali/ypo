...
export function FolderNavigation() {
	...
	const googleDriveParentId =
		useOrganizationFilesStore().use.googleDriveParentId();
	...
	const filesParents = orgFiles.results[0]?.parents as
		| Array<GoogleDriveFileId>
		| undefined;
	const upParent = filesParents?.at(-2);

	const isRoot =
		!googleDriveParentId && (!filesParents || filesParents.length === 0);
