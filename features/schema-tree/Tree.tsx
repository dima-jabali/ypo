...
export function Tree({
	...
	const isOneOfTheParentsOfActiveItem = Boolean(
		expandedItemsRef.current.has(id) &&
	...
	const length =
		(showNumberOfChildren || hasLength || isAParent) && hasLength
			? ` (${children!.length})`
			: null;
	...
	return (
		<>
			<button
				...
				data-is-one-of-the-parents-of-active-item={
					isOneOfTheParentsOfActiveItem
				}
			...
			>
				...
				{length ? (
					<span className="font-mono font-light tabular-nums">{length}</span>
				) : null}
			</button>
