...
export const provideSQLCompletion = (
	monaco: MonacoType,
	...
	for (const suggestion of serverSuggestions) {
		switch (suggestion.suggestion_type) {
			case SuggestionType.DATABASE: {
				...
				const { database_name } = (
					suggestion as Suggestion<SuggestionType.DATABASE>
				).suggestion;
			...
			case SuggestionType.SCHEMATA: {
				const { schemata_name, database_name } = (
					suggestion as Suggestion<SuggestionType.SCHEMATA>
				).suggestion;
			...
			case SuggestionType.KEYWORD: {
				const { keyword } = (suggestion as Suggestion<SuggestionType.KEYWORD>)
					.suggestion;
