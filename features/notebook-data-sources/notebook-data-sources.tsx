...
export function NotebookDataSources() {
	const [selectedDatabase, setSelectedDatabase] =
		useState<NormalDatabaseConnection | null>(null);
	const [openView, setOpenView] = useState(
		OpenView.ALL_DATABASES_AND_DATAFRAMES_AND_COLUMNS,
	);
					...
					<>
						<div className="flex h-fit w-full flex-col gap-1">
							...
							<p className="flex w-[100%] items-center justify-between p-3 text-xs font-light text-primary">
								<span
									className="cursor-pointer onfocus:underline"
									onPointerUp={goToDataframes}
								>
									See all dataframes used in this project
								</span>

								<button
									className="button-hover rounded-sm p-2"
									onPointerDown={goToDataframes}
								>
						...
						<div className="flex flex-col p-3">
							...
							<p className="pt-1 text-xs text-primary">
								Connections to external databases for use by anyone in this
								project.
							</p>
						</div>
	...
	return (
		<>
			...
			<Dialog
				onOpenChange={setIsNewDatabaseModalOpen}
				open={isNewDatabaseModalOpen}
			>
