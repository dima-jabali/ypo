...
export const getInheritanceType = (newValue: unknown) =>
	newValue === SettingValues.Default
		? InheritanceType.DEFAULT
		: InheritanceType.CUSTOM;

export function matchTabToSettingsEntity(
...
export const AGENT_SETTINGS = [
	SettingsKey.REQUIRE_APPROVAL_FOR_CONVERSATION_PLANS,
];
export const ALL_TABS = Object.values(SettingsModalTab);

export function convertTabToSettings(
	tab: SettingsModalTab,
): keyof SettingsReturnType {
	switch (tab) {
		case SettingsModalTab.Organization:
