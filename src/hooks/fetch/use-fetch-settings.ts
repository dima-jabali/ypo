    ...
    (settings: SettingsReturnType) => {
      {
        ...
        function setGeneralSetting(setting: SettingsBase) {
          ...
          // Boolean settings:
          if (BOOLEAN_SETTINGS_TO_BE_SET.includes(setting.key as BooleanSettingsToBeSet)) {
            const key = camelCase(
          ...
          } else if (STRING_SETTINGS_TO_BE_SET.includes(setting.key as StringSettingsToBeSet)) {
            const key = camelCase(setting.key.toLowerCase() as StringSettingsToBeSet) as StringKeys;
