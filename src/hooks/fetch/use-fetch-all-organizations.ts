...
export function useFetchAllOrganizations<SelectedData = GetOrganizationsResponse["results"]>(
  select: (data: GetOrganizationsResponse["results"]) => SelectedData = identity<
...
export function useUserRoleInCurrOrg() {
  ...
  const userRoleInAlreadyDowloadedOrgMembers = useFetchAllOrganizations(selectUserRoleInOrg);

  const orgMember = useOrgMember(userRoleInAlreadyDowloadedOrgMembers === undefined)!;
