...
export function useFetchBotSourcesPage() {
  ...
  const { initialPageParam, queryOptions } = useMemo(() => {
    ...
    const queryOptions = queryKeyFactory.get["bot-sources-page"](organizationId);
  ...
  return useSuspenseInfiniteQuery({
    ...
    ...queryOptions,
    select: (
      data: InfiniteData<GetAllBotSourcesResponse, GetAllBotSourcesRequest> | undefined,
    ) => ({
      results: data?.pages.flatMap((page) => page?.results) ?? DEFAULT_BOT_SOURCES,
