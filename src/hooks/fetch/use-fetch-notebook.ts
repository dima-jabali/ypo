...
export function useFetchNotebook<SelectedData = FetchNotebookResponse>(
  ...
  if (!isValidNumber(notebookId)) {
    ...
    throw new Error(`"notebookId" from general context store is not valid! Got "${notebookId}"`);
  }
...
const selectDownloadedNotebookId = (data: FetchNotebookResponse) => data.metadata.id;
export function useDownloadedNotebookId() {
  return useFetchNotebook(selectDownloadedNotebookId);
}

const selectDownloadedNotebookUuid = (data: FetchNotebookResponse) => data.metadata.uuid;
...
const selectDownloadedNotebookBotConversationId = (data: FetchNotebookResponse) =>
...
const selectDownloadedNotebookMetadata = (data: FetchNotebookResponse) => data.metadata;
export function useDownloadedNotebookMetadata() {
  return useFetchNotebook(selectDownloadedNotebookMetadata);
}

const selectDownloadedNotebookStatus = (data: FetchNotebookResponse) => data.metadata.status;
export function useDownloadedNotebookStatus() {
  return useFetchNotebook(selectDownloadedNotebookStatus);
}

const selectDownloadedNotebookTags = (data: FetchNotebookResponse) => data.metadata.tags;
export function useDownloadedNotebookTags() {
  return useFetchNotebook(selectDownloadedNotebookTags);
}

const selectDownloadedNotebookTitle = (data: FetchNotebookResponse) => data.metadata.title;
...
export function useNotebookBlock(uuid: NotebookBlockUuid) {
  const selectBlock = useCallback(
    (data: FetchNotebookResponse) => data.blocks.find((block) => block.uuid === uuid),
    [uuid],
  );
...
function selectSortedNotebookBlocks(data: FetchNotebookResponse): NotebookBlock[] {
  ...
  if (!firstBlockUuid) {
    throw new Error("First block (block_above_uuid === null) not found in a non-empty notebook!");
  }
...
export function useVariablesToSelect(blockWriteVariables: BlockBase["write_variables"]) {
    ...
    (data: FetchNotebookResponse) => {
      ...
      for (const key in projectVariableInfo) {
        ...
        if (
          ...
          variable.type ===
            (SqlBlockSourceType.Dataframes as unknown as NormalDatabaseConnection["type"]) ||
          blockWriteVariables?.some((tableBlockVariable) => tableBlockVariable.name === key)
        ) {
          continue;
        }
