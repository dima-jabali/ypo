...
import type { PageLimit, PageOffset } from "#/contexts/general-ctx/general-context";
import {
  GET_AWS_FILE_AS_STRING_BINARY_ACTION,
  getErrorMessage,
  isValidNumber,
} from "#/helpers/utils";
import { DocumentSource, GeneralFileType, type GeneralFile } from "#/types/notebook";
...
export async function queryFnToFetchFileById({
  ...
  try {
    ...
    if (hasValidFileId) {
      const generalFileResponse = await clientAPI_V1.get<GeneralFile>(`/files/${fileId}`);
      ...
      if (generalFileResponse.data.document_source === DocumentSource.GOOGLE_DRIVE) {
        throw new Error("Google Drive files cannot be downloaded, only opened in a new tab!");
      }
      ...
      const generalFilesPageResponse = await clientAPI_V1.get<GetGeneralFilesPageResponse>(
