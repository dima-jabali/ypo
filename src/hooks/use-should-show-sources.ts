...
import { BotConversationMessageType, type ParallelConversationId } from "#/types/chat";

export function useShouldShowSources(parallelConversationId: ParallelConversationId | null) {
  const shouldShowSourcesBasedOnSettings = generalContextStore.use.showInternalSources();
  ...
  const { getBotConversationMessageListPages, botConversationId } = generalContextStore.getState();

  const chatMessagesSoFar = getBotConversationMessageListPages(botConversationId!)?.pages;
