...
export function useUpdateNotebookMetadata() {
	...
	>({
		...
		async onSuccess(updatedNotebookFromResponse, args) {
			const { setNotebookListPages, setNotebook } =
			...
			setNotebookListPages(
				...
				(cachedProjectsInfiniteQueryResponse) => {
					if (
						!(
							cachedProjectsInfiniteQueryResponse && updatedNotebookFromResponse
						)
					) {
						console.log(
					...
					const prevResults =
						cachedProjectsInfiniteQueryResponse.pages[path.pagesIndex]?.results;
					...
					const newPage: (typeof cachedProjectsInfiniteQueryResponse.pages)[number] =
						{
							...oldPage,
							num_results: shouldRemoveFromList
								? oldPage.num_results - 1
								: oldPage.num_results,
							results: newResults,
						};
