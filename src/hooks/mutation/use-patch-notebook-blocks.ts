...
export function setMutationDefaults_patchNotebookBlocks(queryClient: QueryClient) {
  queryClient.setMutationDefaults(mutationKey, {
    mutationFn: async (args: PatchNotebookRequest) => {
      ...
      const res = await clientAPI_V1.patch<PatchProjectResponse>(`/projects/${notebookId}`, body);
