...
type RunPythonResponse = PostBlockResponse<PaginateDataframeOutput<KernelResult>>;
...
export function useRunPython(blockUuid: NotebookBlockUuid) {
  ...
  return useMutation<RunPythonResponse, Error, RunPythonRequest>({
    mutationKey: queryKeyFactory.post["block-request"]._ctx["run-python"](blockUuid).queryKey,
    ...
    mutationFn: async (args) => {
      ...
      const res = await clientAPI_V1.post<RunPythonResponse>(`/blocks/${blockUuid}/action`, action);
