...
import type { AwsBucket, AwsKey, Organization } from "../fetch/use-fetch-all-organizations";
...
export function useMutateOrganization() {
  return useMutation({
    ...
    mutationFn: async (args: MutateOrganizationRequest) => {
      ...
      const res = await clientAPI_V1.put<MutateOrganizationResponse>(path, args.body);
