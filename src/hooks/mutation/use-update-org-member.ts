...
import type { BetterbrainUserId, OrganizationMemberRole } from "#/types/notebook";
...
const ALL_ORGANIZATIONS_QUERY_KEY = queryKeyFactory.get["all-organizations"].queryKey;
...
export const useUpdateOrgMember = () => {
  ...
  queryClient.setMutationDefaults(mutationKey, {
    onSuccess: async () => await queryClient.invalidateQueries(cancelOrInvalidateQueriesParams),
    onMutate: async () => await queryClient.cancelQueries(cancelOrInvalidateQueriesParams),
  } satisfies MutationObserverOptions<UpdateOrgMemberResponse, Error, UpdateOrgMemberRequest>);
  ...
  return useMutation<UpdateOrgMemberResponse, Error, UpdateOrgMemberRequest>({
    ...
    mutationFn: async (args: UpdateOrgMemberRequest) => {
      ...
      return await clientAPI_V1.put<UpdateOrgMemberRequest, UpdateOrgMemberResponse>(
        path,
