...
const mutationKey = queryKeyFactory.post["block-request"]._ctx["generate-sql-description"].queryKey;

export function useGenerateSqlDescription() {
  return useMutation<GenerateSqlDescriptionResponse, Error, GenerateDescriptionRequest>({
    mutationKey,
    ...
    mutationFn: async (args) => {
      ...
      const res = await clientAPI_V1.post<GenerateSqlDescriptionResponse>(path, paginateAction);
