import { useMutation, type InvalidateQueryFilters, type QueryFilters } from "@tanstack/react-query";
...
export type SetSettingsRequest = {
  ...
  updates: Array<{
    value: string | number | boolean | Array<unknown> | Record<string, unknown> | undefined;
...
const INITIAL_PART_OF_SETTINGS_QUERY_KEY = queryKeyFactory.get["settings"]._def.join("");
const cancelOrInvalidateQueriesParams: QueryFilters | InvalidateQueryFilters = {
  predicate(query) {
    return query.queryKey.slice(0, 2).join("") === INITIAL_PART_OF_SETTINGS_QUERY_KEY;
  },
};
