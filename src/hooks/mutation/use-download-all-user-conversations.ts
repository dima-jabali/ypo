...
import { GET_AWS_FILE_AS_STRING_BINARY_ACTION, isValidNumber } from "#/helpers/utils";
...
export function useDownloadAllUserConversations() {
  return useMutation<DownloadAllConversationsResponse, Error, DownloadAllConversationsRequest>({
    mutationKey,
    ...
    mutationFn: async (args) => {
      ...
      const responseAws = await clientAPI_V1.get<DownloadAllConversationsAwsResponse>(path);
