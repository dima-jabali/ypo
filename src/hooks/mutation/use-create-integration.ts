...
export function useCreateIntegration<T extends DatabaseConnectionType>() {
  ...
  return useMutation<CreateIntegrationResponse, Error, CreateIntegrationRequest>({
    mutationKey,

    mutationFn: async (body) => {
      const res = await clientAPI_V2.post(`/organizations/${organizationId}/integrations`, body);
