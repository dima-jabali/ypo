...
type UpdateIntegrationRequest<T extends DatabaseConnectionType> = BaseUpdateIntegrationRequest &
...
export function useUpdateIntegration<T extends DatabaseConnectionType>() {
  ...
  return useMutation<UpdateIntegrationResponse<T>, Error, UpdateIntegrationRequest<T>>({
    mutationKey,
    ...
    meta: {
      invalidateQuery: queryKeyFactory.get["all-database-connections"](organizationId),
      cancelQuery: queryKeyFactory.get["all-database-connections"](organizationId),
