...
import type { BetterbrainUserId, NotebookBlockUuid, PdfId } from "#/types/notebook";
...
export const queryKeyFactory = createQueryKeyStore({
  get: {
    ...
    "org-member": (organizationId: OrganizationId, userId: BetterbrainUserId) => ({
    ...
    "bot-plan": (
      ...
      queryFn: () => api.get["bot-plan"](botConversationId, organizationId, notebookId),
    ...
    "pdf-or-csv-files-page": (organizationId: OrganizationId, file_type: PDFOrCSVSourceType) => ({
    ...
    "all-organization-files": (
      ...
      queryFn: () => api.get["all-organization-files"](organizationId, queryParams),
      queryKey: [organizationId, queryParams],
    }),

    "bots-page": (organizationId: OrganizationId) => ({
      queryFn: ({ pageParam, signal }: { pageParam: GetAllBotsRequest; signal: AbortSignal }) =>
    ...
    "bot-conversation-message-list-page": (botConversationId: BotConversationId) => ({
      queryFn: ({ pageParam }: { pageParam: GetBotConversationMessagesPageRequest }) =>
    ...
    settings: (organizationId: OrganizationId, notebookId: NotebookId | undefined) => ({
