...
export function useSetNotebookToFirst() {
  ...
  return useQuery({
    enabled: !hasNotebookSelected && hasNotebooks && isValidNumber(organizationId),
    ...
    queryFn: async (): Promise<HasSetNotebookToFirst> => {
      ...
      if (!isValidNumber(firstNotebookMetadata.id)) {
        console.log("No first notebook id! Seems that this notebook is an optimistic one", {
        ...
        const newNotebookList: FetchNotebookListPageResponse = await queryClient.fetchQuery({
          ...queryOptions,
      ...
      console.log("No notebook selected, setting first notebook", firstNotebookMetadata);
