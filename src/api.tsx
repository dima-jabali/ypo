...
import { databasesSchemaStore, type DatabaseConnectionsSchema } from "#/contexts/databases-schema";
...
import type { GetAllBotsRequest, GetAllBotsResponse } from "#/hooks/fetch/use-fetch-bots-page";
import type {
...
export async function validateTokenToSetItOnApiHeader(token: string) {
  try {
    const response = await fetch(`${BACKEND_API}/api/v1/validate-short-lived-token`, {
...
clientAPI_V1.interceptors.request.use(putAuthTokenOnHeader, putAuthTokenOnHeaderError);
clientAPI_V1.interceptors.response.use(errorsAndNotificationsOnResponse, handleNetworkErrors);
clientAPI_V2.interceptors.request.use(putAuthTokenOnHeader, putAuthTokenOnHeaderError);
clientAPI_V2.interceptors.response.use(errorsAndNotificationsOnResponse, handleNetworkErrors);
clientBareAPI.interceptors.request.use(putAuthTokenOnHeader, putAuthTokenOnHeaderError);
clientBareAPI.interceptors.response.use(errorsAndNotificationsOnResponse, handleNetworkErrors);
...
export const api = {
  get: {
    "org-member": async (organizationId: OrganizationId, userId: BetterbrainUserId) => {
    ...
    "bot-plan": async (
      ...
      const sortedSubSteps = plan?.sub_tasks?.toSorted((a, b) => a.step_num - b.step_num);

      const newPlan = plan ? ({ ...plan, sub_tasks: sortedSubSteps } satisfies Plan) : null;
    ...
    "all-organization-files": async (
      ...
      try {
        ...
        const searchParams = new URLSearchParams(queryParams as any as Record<string, string>);
    ...
    "database-data": async (dbParams: FetchDatabaseDataParams) => {
      try {
        const entity_type =
          "entity_type" in dbParams ? dbParams.entity_type : EntityType.DATA_SCHEMA;
        const entity_id = "entity_id" in dbParams ? dbParams.entity_id : dbParams.db.data_schema_id;
        ...
        const queryResponse = await clientBareAPI.post<PostQueryResponse>("/v2/query", queryBody);
        ...
        if (!(schema && connectionType)) {
          ...
          toast({
            description: (
              <p>
                <span className="italic underline underline-offset-2">{dbParams.db.name}</span> — we
                don&apos;t have the schema at all, so there are no tables to show!
              </p>
            ),
        ...
        databasesSchemaStore.setState((prev) => {
          ...
          const dbIndex = prev.databasesSchema.findIndex(
            (item) =>
              item.databaseId === dbParams.db.id && dbParams.db.type === item.connectionType,
          );
          ...
          if (dbIndex === -1) {
            ...
            const mergedSchemaData = mergeSchemaData(prev.databasesSchema, dbIndex, newSchema);
      ...
      try {
        ...
        queryClient.setQueryData<Array<Organization>>(
          ...
          (prevAllOrganizations) => {
            ...
            const prevOrganization = prevAllOrganizations[prevOrganizationIndex];

            if (!prevOrganization) return prevAllOrganizations;

            const nextAllOrganizations: typeof prevAllOrganizations = [...prevAllOrganizations];
    ...
    "all-organizations": async () => {
      try {
        const res = await clientAPI_V1.get<GetOrganizationsResponse>("/organizations");
    ...
    "all-database-connections": async (organizationId: OrganizationId) => {
      ...
      const isNormalDatabaseConnection = (db: DatabaseConnection): db is NormalDatabaseConnection =>
      ...
      ): db is SlackConnectionDataWithDefinedChannels => db.type === DatabaseConnectionType.Slack;

      const isPlaidConnection = (db: DatabaseConnection): db is PlaidConnection =>
        db.type === DatabaseConnectionType.Plaid;

      const isClickUpConnection = (db: DatabaseConnection): db is ClickUpConnectionType =>
        db.type === DatabaseConnectionType.ClickUp;

      const isAirtableDatabaseConnection = (
        db: DatabaseConnection,
      ): db is AirtableDatabaseConnection => db.type === DatabaseConnectionType.Airtable;

      const isGoogleDriveDatabaseConnection = (
        db: DatabaseConnection,
      ): db is GoogleDriveDatabaseConnection => db.type === DatabaseConnectionType.GoogleDrive;

      const store: AllDatabaseConnections = {
        allDatabaseConnections,

        // Derived values from `allDatabaseConnections`:
        airtableDatabaseConnections: allDatabaseConnections.filter(isAirtableDatabaseConnection),
        normalDatabases: allDatabaseConnections.filter(isNormalDatabaseConnection),
        googleDriveDatabases: allDatabaseConnections.filter(isGoogleDriveDatabaseConnection),
    ...
    "bot-conversation-message-list-page": async (
      ...
      const res = await clientAPI_V1.get<GetBotConversationMessagesPageResponse>(
    ...
    "notebook-by-id": async (notebookId: NotebookId) => {
      if (!isValidNumber(notebookId)) {
        throw new Error(`notebookId is not valid. Expected a number, got ${notebookId}`);
      }

      const start = performance.now();

      const res = await clientAPI_V1.get<FetchNotebookResponse>(`/projects/${notebookId}`);

      log(`useFetchNotebook(notebookId = ${notebookId}) took ${performance.now() - start}ms`);
    ...
    settings: async (organizationId: OrganizationId, notebookId: NotebookId | undefined) => {
    ...
    "pdf-file-by-id": async (pdfFileId: FileId | PdfId | undefined) => {
      try {
        const presignedUrlResponse = await clientAPI_V1.get<GetPresignedUrlByFileIdResponse>(
          `/files/${pdfFileId}`,
        );
