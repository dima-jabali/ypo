...
import type { BotConversationMessage, BotConversationMessageUuid, Plan, PlanStep } from "./chat";
import type { ISODateString, StreamUuid, UUID } from "./general";
import type { BetterbrainUser, NotebookBlock, NotebookBlockUuid } from "./notebook";
...
export type UpdateBotConversationMessageResponseAction = {
  ...
  action_payload: {
    ...
    key: string[];
  } & UpdateActionValue<string | Record<string, string> | unknown[] | number | boolean>;
};
...
export type RunTableBlockResponse<T = unknown> = PostBlockResponse<PaginateDataframeOutput<T>>;
