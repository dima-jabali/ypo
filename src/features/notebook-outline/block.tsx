...
function getBlockJSX({
	...
	switch (projectBlock.type) {
		...
		case BlockType.Csv: {
			...
			const dataPreview = (projectBlock as BlockSql | BlockCsv)
				.custom_block_info?.data_preview;
					...
					? (() => {
							const dataToUse = (() => {
								if (dataPreview.data.length > MIN_NUMBER_OF_ROWS_FOR_OUTLINE) {
									return dataPreview.data.slice(
										0,
										MIN_NUMBER_OF_ROWS_FOR_OUTLINE,
									);
								}
							...
							const tableMapData = new Map(
								dataToUse.entries(),
							) as TableDataType["allData"];
...
const DEFAULT_WRITE_VARS: NonNullable<
	BlockProps["projectBlock"]["write_variables"]
> = [];
...
export function Block({
	...
	const isRunningBlock =
		runPython.isPending || runCsv.isPending || runSql.isPending;
			...
			switch (projectBlock.type) {
				case BlockType.Sql: {
					runSql.mutate({
						action_info: {
							filters:
								projectBlock.custom_block_info?.filters || DEFAULT_FILTERS,
