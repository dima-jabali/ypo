...
export const makeSortTextStringsAndInserThem = <T extends { sortText?: string }>(
  arrayToChange: T[],
) => {
  ...
  for (let uniqueStringIndex = 0; uniqueStringIndex < numberOfUniqueStrings; ++uniqueStringIndex) {
    ...
    if (
      uniqueStringIndex !== 0 &&
      isLeftMultipleOfRight(uniqueStringIndex + 1, SORTED_LOWERCASE_ALPHABET_LENGHT)
    ) {
      charOnAlphabet = 0;
      ++position;
    }
