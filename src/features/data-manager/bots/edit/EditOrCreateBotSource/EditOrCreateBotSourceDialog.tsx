...
import { BotSourceFormAction, BotSourceType, type BotSource } from "#/types/bot-source";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
type Props = {
  ...
  className?: string;
  source: BotSource;
  setBotSourceBeingEditedOrAdded: React.Dispatch<React.SetStateAction<BotSource | null>>;
  setNextBotSources: React.Dispatch<React.SetStateAction<BotSource[]>>;
  closeDialog: () => void;
};

type MatchBotSourceEditFormProps = Omit<Props, "setBotSourceBeingEditedOrAdded">;
...
export function EditOrCreateBotSourceDialog({
  ...
  const cannotChangeBotType =
    initialValue.source_type === BotSourceType.Web && action === BotSourceFormAction.Create;
      ...
      >
        ...
        >
          <label className="flex flex-col gap-1">
            ...
            <Select
              disabled={action === BotSourceFormAction.Edit || cannotChangeBotType}
          ...
          <label className="flex flex-col gap-1">
            ...
            <Input name={BOT_NAME_INPUT_NAME} defaultValue={source.name} required />
          </label>
          ...
          {action === BotSourceFormAction.Edit ? (
            <fieldset className="flex flex-col gap-1">
              ...
              <Switch defaultChecked={source.archived} name={IS_BOT_SOURCE_ARCHIVED_INPUT_NAME} />
            </fieldset>
          ) : null}
