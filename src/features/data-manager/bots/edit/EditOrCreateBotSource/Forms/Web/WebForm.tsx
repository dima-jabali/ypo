...
export const WebForm: React.FC<Props> = ({ action, source, setNextBotSources, closeDialog }) => {
  const organizationId = generalContextStore.use.organizationId();

  const [selectedWebCrawls, setSelectedWebCrawls] = useState(source.web_crawls ?? []);
  ...
  async function handleSendWebForm() {
    ...
    try {
      ...
      const createOrEditWebsitesPromises = websitesRef.current
        .filter((item) => !source.websites?.find((website) => website.id === item.id))
      ...
      const createdOrEditedWebsites = await Promise.all(createOrEditWebsitesPromises);
    ...
    <>
      <section className="flex flex-col gap-1 whitespace-nowrap rounded-lg border border-border-smooth  p-2">
        <p className="pl-2 font-bold">
          Websites
          <span className="font-normal tabular-nums"> ({websitesRef.current.length})</span>:
        </p>
        ...
        <ul className="flex flex-col items-center gap-1">
          {websitesRef.current.map((website, urlIndex) => (
            <WebsiteCard
              ...
              isOnSource={!!source.websites?.find(({ id }) => id === website.id)}
      ...
      <section className="flex flex-col gap-1 rounded-lg border border-border-smooth  p-2">
        <div className="flex w-full items-center justify-between">
          <p className="pl-2 font-bold">
            Web Crawls
            <span className="font-normal tabular-nums"> ({selectedWebCrawls.length})</span>:
          </p>
