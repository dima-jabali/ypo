...
export const PDF_Or_CSV_Form: React.FC<Props> = ({
  ...
  const [selectedPDFOrCSVFiles, setSelectedPDFOrCSVFiles] = useState<PDFWithNameKey[]>([]);
  ...
  useEffect(() => {
    ...
    files.map((file, index) => {
      if (prevFiles.find((prevFile) => prevFile.previewURL === file.previewURL)) {
    ...
    const handleDroppedFiles = (e: DragEvent) => {
      ...
      setFiles((prev) => {
        ...
        for (const file of files) {
          if (
            (source.source_type === BotSourceType.CSV && file.type !== "text/csv") ||
            (source.source_type === BotSourceType.PDF && file.type !== "application/pdf")
          )
            continue;
  ...
  const handleRemoveFile = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>, index: number) => {
    e.stopPropagation();
  ...
  const handleOpenFileInputChooserByPressingEnter = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      document.getElementById(PDF_OR_CSV_FILE_INPUT_ID)?.click();
    }
  };
  ...
  const handleToggleSelectAllFiles = () => {
    setSelectedPDFOrCSVFiles(
      selectedPDFOrCSVFiles.length === allPDFOrCSVFiles.length ? [] : allPDFOrCSVFiles,
    );
  };
    ...
    try {
      ...
      const promisesOfUploadUrls = files.map(({ file }) =>
        clientAPI_V1
          ...
          .catch((error) => {
            console.error(`Error getting upload URL for ${source.source_type} file`, error);
      ...
      const responsesOfUploadUrls = await Promise.allSettled(promisesOfUploadUrls);
      ...
      const promisesOfFilesUpload = responsesOfUploadUrls.map(async (res) => {
        ...
        if (!fileHolder) {
          console.error("Could not find file to upload! This should not happen!", {
            files,
            res,
          });
        ...
        return axios
          .put(res.value.data.upload_url, fileHolder.file, {
            headers: {
              "Content-Type":
                source.source_type === BotSourceType.CSV ? "text/csv" : "application/pdf",
            },
          })
          .catch((error) => {
      ...
      const responsesOfUploadFiles = await Promise.allSettled(promisesOfFilesUpload);
        ...
        <TagGroup
          ...
          footer={
            ...
            <RadioGroup
              ...
              value={
                selectedPDFOrCSVFiles.length === allPDFOrCSVFiles.length ? "All files" : undefined
              }
            >
              <label className="flex items-center gap-2 p-4 onfocus:underline active:brightness-150">
                <RadioGroupItem
                  defaultChecked={selectedPDFOrCSVFiles.length === allPDFOrCSVFiles.length}
...
export const renderRemovableFileItem: TagGroupProps<PDFWithNameKey>["renderRemovableItem"] = (
  ...
  <div /* Selected item container */
    ...
    key={item.id}
  >
    <p className="max-w-52 truncate whitespace-nowrap px-2" title={`${item.id} | ${item.name}`}>
      {item.name}
    </p>
