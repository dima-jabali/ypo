...
const CONNECT_TO_SLACKS_WORKSPACE_URL = process.env.NEXT_PUBLIC_CONNECT_TO_SLACKS_WORKSPACE_URL;
...
export function DBConnectionModalContent({
  ...
  const [databaseToConnect, setDatabaseToConnect] = useState<DatabaseConnectionType | undefined>(
    dbToCreate,
  );
  const [botSourceBeingCreated, setBotSourceBeingCreated] = useState<BotSource | null>(null);
  ...
  const createGoogleDriveIntegration = useCreateIntegration<DatabaseConnectionType.GoogleDrive>();
  const createAirtableIntegration = useCreateIntegration<DatabaseConnectionType.Airtable>();
  const createPostgresIntegration = useCreateIntegration<DatabaseConnectionType.Postgres>();
  ...
  useEffect(() => {
    setDatabaseName(connectionsData.find((item) => item.kind === databaseToConnect)?.name ?? "");
  }, [databaseToConnect]);

  useEffect(() => {
    if (!attemptToConnectStatus) return;

    if (attemptToConnectStatus.dialogToOpenWithError !== DialogToOpenWithError.none) {
  ...
  async function handleConnectToYouTube() {
    ...
    try {
      const connectToYouTubeResponse = await clientAPI_V1.get<ConnectToYouTubeResponse>(
  ...
  async function handleConnectToAirtableWorkspace() {
    ...
    const formData = new FormData(formRef.current);
    const formEntries = Object.fromEntries(formData);
    const name = formEntries[AIRTABLE_CONNECTION_NAME_INPUT_ID] as string | undefined;
    const airtablePAT = formEntries[AIRTABLE_PAT_INPUT_ID] as string | undefined;
  ...
  async function handleConnectToGoogleDrive() {
    ...
    try {
      const serviceAccountInfoAsString = formEntries["service_account_info"] as string;
  ...
  return (
    <>
      ...
      <DialogDescription>Choose a database to create a connection with:</DialogDescription>
      ...
      <p className="text-center">
        ...
        <a className="link hover:underline" href="mailto:support@betterbrain.ai">
          Let us know
        </a>
      </p>
      ...
      <Dialog onOpenChange={setIsErrorDialogOpen} open={isErrorDialogOpen}>
        <DialogContent className="flex flex-col overflow-auto border-border-smooth p-9 pb-6 z-500">
          ...
          <DialogDescription>
            We could not create your connection. We&apos;ve logged the error and are working on it.
          </DialogDescription>

          <p className="mt-5">{attemptToConnectStatus?.error.replaceAll('"', "")}</p>
