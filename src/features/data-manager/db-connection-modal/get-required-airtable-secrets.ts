...
async function generateCodeChallenge(codeVerifier: string): Promise<string> {
  // Hash code_verifier
  const sha256Hash = window.crypto.subtle.digest("SHA-256", new TextEncoder().encode(codeVerifier));
...
async function getRequiredSecrets() {
  ...
  const codeVerifier = generateSecureString(Math.floor(Math.random() * (129 - 43) + 43));
  ...
  // Generate state
  const state = generateSecureString(Math.floor(Math.random() * (1025 - 16) + 16));
