...
export const ChannelsToUseAsBotSourceSelector: React.FC<Props> = ({
  ...
  const [allNonDirectMessagesChannels, setAllNonDirectMessagesChannels] = useState<
    SelectedChannel[]
  >([]);
  const [radioSelected, setRadioSelected] = useState(RadioSelected.NoOptionSelected);
  ...
  useLayoutEffect(() => {
    const nextAllPublicChannels = allChannels.filter((channel) => !channel.is_private);
          ...
          footer={
            <>
              ...
              <RadioGroup className="flex flex-col gap-0 mb-1 mt-2 *:px-4 *:py-1 *:gap-2 text-xs">
                ...
                <label className="flex items-center onfocus:underline active:brightness-150 button-hover rounded">
                  <RadioGroupItem
                    checked={radioSelected === RadioSelected.AllNonDirectMessagesChannels}
              ...
              <div className="my-1 flex flex-col gap-1 px-1">
                ...
                >
                  Exclude all <i className="font-bold">private</i> channels from current selection
                </button>

                <button
                  className="px-3 py-2 button-hover rounded-md border border-border-smooth  text-xs text-start"
                  onClick={handleExcludeAllDirectMessageChannelsFromCurrentSelection}
                >
                  Exclude all <i className="font-bold">direct message</i> channels from current
                  selection
                </button>
                ...
                >
                  Exclude all <i className="font-bold">externally shared</i> channels from current
                  selection
                </button>
              </div>
...
const renderItemWhenDisabled: TagGroupProps<SelectedChannel>["renderItemWhenDisabled"] = (item) => (
