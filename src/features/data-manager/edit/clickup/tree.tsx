...
export const Tree = memo(function Tree({ clickUpDatabaseConnection, updatesRef }: Props) {
...
type Item = ClickUpWorkspace | ClickUpChatView | ClickUpFolder | ClickUpSpace | ClickUpList;

const getIsWorkspace = (item: Item): item is ClickUpWorkspace => "workspace_id" in item;
const getIsChatView = (item: Item): item is ClickUpChatView => "view_id" in item;
...
function getOrCreateUpdateItem(
  ...
  const existingUpdateItem = updatesRef.current.find(
    ({ entity_id, entity_type }) => entity_id === itemId && entity_type === entityType,
  );
  ...
  if ("index_documents" in item) {
    newUpdateItem.index_documents_for_children_by_default = item.index_documents_by_default;
...
const TreeItem: React.FC<{
  item: ClickUpWorkspace | ClickUpChatView | ClickUpFolder | ClickUpSpace | ClickUpList;
  updatesRef: React.RefObject<Array<ClickUpConnectionFieldsUpdate>>;
  clickUpConnectionId: ClickUpConnectionId;
  organizationId: OrganizationId;
}> = memo(function TreeItem({ item, clickUpConnectionId, updatesRef, organizationId }) {
  const { id, name } = item;

  const [{ params, hasChildren, ButtonOrDiv, icon, friendlyItemType }] = useState(() => {
    ...
    const icon =
      "avatar" in item && item.avatar ? (
        <img className="rounded-full flex-none" src={item.avatar} height={20} width={20} alt="" />
      ) : (
  ...
  const handleToggleIndexChildrenDocumentsByDefault = (e: CheckedState) => {
    itemWithUpdates.index_documents_for_children_by_default = e.valueOf() as boolean;
            ...
            {itemWithUpdates.index_documents && hasChildren ? (
              <>
                ...
                <label className="flex items-center h-full gap-2">
                  ...
                  <Checkbox
                    checked={itemWithUpdates.index_documents_for_children_by_default ?? false}
                    onCheckedChange={handleToggleIndexChildrenDocumentsByDefault}
                  />
                </label>
              </>
            ) : null}
            ...
            {isChanging && hasChildren ? (
              <>
                ...
                <label className="flex items-center h-full gap-2">
                  ...
                  <Checkbox
                    checked={itemWithUpdates.select_index_documents_recursive ?? false}
