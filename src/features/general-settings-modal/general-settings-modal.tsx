...
import { useSetSettings, type SetSettingsRequest } from "#/hooks/mutation/use-set-settings";
...
export const GeneralSettingsModal = memo(function GeneralSettingsModal() {
  const [activeSettingOption, setActiveSettingOption] = useState(SettingsOptions.General);
  ...
  settings[convertTabToSettings(activeTab)]?.forEach((setting) => {
    ...
    if (setting.details.show_as_user_setting === false && activeTab === SettingsModalTab.User) {
      return;
    }
  ...
  async function handleSaveChanges() {
    ...
    try {
      ...
      const newSettings: SetSettingsRequest = {
        project_id: activeTab === SettingsModalTab.Project ? notebookId : undefined,
  ...
  function renderSettingOptions(setting: SettingsBase) {
    switch (setting.details.type) {
      case DetailsType.BOOLEAN: {
        return (
          <BooleanRadio handleOnChange={handleOnChange} activeTab={activeTab} setting={setting} />
        );
      }

      case DetailsType.ENUM: {
        return (
          <EnumRadio handleOnChange={handleOnChange} activeTab={activeTab} setting={setting} />
        );
      }
      ...
      case DetailsType.STRING: {
        return <StringInput handleOnChange={handleOnChange} setting={setting} />;
      }
    ...
    return (
      ...
      >
        <div className="flex gap-2 flex-col">
          ...
          <p className="text-xs text-muted-foreground">{setting.details.description}</p>
        </div>
        ...
        {isOpen ? (
          <>
            ...
            <p className="text-xs text-muted-foreground mt-1.5">
              Project&apos;s settings overrides User&apos;s, which overrides Organization&apos;s.
            </p>
