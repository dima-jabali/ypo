...
export function StandardDocumentMinimalDetails({ normalizedSource }: MinimalProps) {
    ...
    >
      <summary className="cursor-pointer max-h-[4lh] overflow-hidden text-xs">
        <span className="group-open:hidden">
          <HighlightStringWithFilterRegex string={values.fields.long_text_data?.[0] || ""} />
        </span>
      </summary>
      ...
      <div className="flex flex-col gap-2 w-full h-full text-xs">
        {isOpen ? (
          <>
            ...
            <HighlightStringWithFilterRegex string={values.fields.long_text_data?.join("") || ""} />
          </>
        ) : null}
      </div>
    </details>
...
export function StandardDocumentVerboseDetails({ normalizedSource }: VerboseProps) {
  ...
  return (
    <details onToggle={remeasureHeight}>
      ...
      >
        ...
        {values.metadata ? <FileMetadataTable metadata={values.metadata} /> : null}
      </summary>
        ...
        {values.content_list?.map((item, index) => {
          ...
          switch (item.type) {
            case StandardDocumentContentType.Text: {
              if (item.text?.startsWith("START") || item.text?.startsWith("END")) {
                return null;
              }
