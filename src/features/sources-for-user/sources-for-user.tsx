...
function Sources({ shouldShow, sources }: { sources: Array<SourceForUser>; shouldShow: boolean }) {
  ...
  const mostRelevantMainValues = sourcesMainValues.slice(0, shouldShowAllSourcesSidebar ? 8 : 9);

  const mostRelevantMainValuesJSXs = mostRelevantMainValues.map((sourceMainValues) => (
    <HoverSourceSnippet sourceMainValues={sourceMainValues} key={sourceMainValues.id} />
  ));
      ...
      <FilterRegexProvider>
        <ul className="sources-wrapper my-4 empty:hidden flex flex-wrap gap-2 max-w-full text-sm tabular-nums text-primary">
          ...
          {shouldShowAllSourcesSidebar ? (
            <AllSourcesInDrawer sourcesMainValues={sourcesMainValues} shouldShow />
          ) : null}
        </ul>
      </FilterRegexProvider>
    </DefaultSuspenseAndErrorBoundary>
