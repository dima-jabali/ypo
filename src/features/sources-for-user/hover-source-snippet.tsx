...
export const HoverSourceSnippet = memo(function HoverSourceSnippet({
	sourceMainValues,
}: Props) {
	const { titleString, titleJSX, descriptionString, descriptionJSX } =
		sourceMainValues;
	...
	return (
		<HoverCard {...props}>
			...
			<HoverCardContent
				className="flex flex-col w-72 max-h-72 p-0"
				sideOffset={5}
				side="top"
			>
				<header className="flex items-start justify-between gap-3 w-full p-3">
					<p
						className="line-clamp-2 text-sm font-bold text-primary"
						title={titleString}
					>
						{titleString}
					</p>
				...
				<footer className="flex flex-col gap-1 px-3 py-2 text-xs">
					<section className="flex gap-2 justify-between">
						...
						<i className="text-primary">
							{titleCase(
								sourceMainValues.normalizedSource.source_type.toLowerCase(),
							)}
						</i>
					</section>
				</footer>
