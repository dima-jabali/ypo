...
> = {
  normalizedSource: Extract<NormalizedSource, { source_type: SourceType; values_type: ValuesType }>;
...
export function getMinimalStandardDocumentValues(
  ...
  >,
): SourceMainValues<SourceForUserType.StandardDocument, StandardDocumentSourceType.Minimal> {
  ...
  clickUpTask: if (
    ...
    > = {
      ...
      descriptionString: removeSourceCitations(values.fields.long_text_data?.join("") || ""),
  ...
  clickUpComment: if (documentType === DocumentType.ClickUpComment) {
    ...
    const commentId = id.slice(clickUpCommentIndex + CLICKUP_COMMENT_SUBSTRING.length);
    const titleString = removeSourceCitations(values.fields.filename || "");
  ...
  if (documentType === DocumentType.ClickUpDocument) {
    ...
    const documentId = values.fields.folder_path_ids
      ?.find((str) => str.startsWith("DOCUMENT_") && !str.startsWith("DOCUMENT_PAGE_"))
      ?.replace("DOCUMENT_", "");
  ...
  if (documentSource === DocumentSource.Clickup) {
    ...
    hrefForTask = taskIdInPath ? `${CLIKUP_TASK_LINK}${taskIdInPath}` : undefined;
  }
...
export function getVerboseStandardDocumentValues(
  ...
  >,
): SourceMainValues<SourceForUserType.StandardDocument, StandardDocumentSourceType.Verbose> {
  const { values } = normalizedSource;
  const descriptionString = removeSourceCitations(
    values.content_list
      ?.map((item) => (item.type === StandardDocumentContentType.Text ? item.text : ""))
      .join("") || "",
  );
...
export function getSourceMainValues(
  ...
  switch (normalizedSource.source_type) {
    case SourceForUserType.Website: {
      ...
      return {
        get titleJSX() {
          const value = <Link href={values.link ?? values.identifier} title={values.identifier} />;
    ...
    case SourceForUserType.GoogleDrive: {
      switch (normalizedSource.values_type) {
        case GoogleDriveSourceType.Minimal: {
          const titleString = normalizedSource.values.fields.filename || "File on Google Drive";
          ...
          return {
            descriptionString: removeSourceCitations(
              normalizedSource.values.content_list
                .map((item) => (item.type === GoogleDriveContentType.Text ? item.text : ""))
                .join("") || "",
            ),
            id:
    ...
    case SourceForUserType.Airtable: {
      ...
      return {
        ...
        get descriptionJSX() {
          const value = <AirtableDescription normalizedSource={normalizedSource} />;
        ...
        get titleJSX() {
          const value = <Link href={values.link ?? values.table_name} title={values.table_name} />;
    ...
    case SourceForUserType.Slack: {
      ...
      return {
        ...
        normalizedSource,
        get descriptionJSX() {
          const value = <SlackConversations normalizedSource={normalizedSource} />;
        ...
        get descriptionJSX() {
          const value = (
            <div className="text-xs">
              <HighlightStringWithFilterRegex string={values.text.slice(0, 100)} />
            </div>
          );
    ...
    case SourceForUserType.Web: {
      ...
      return {
        ...
        get titleJSX() {
          const value = <Link href={values.link ?? values.url} title={values.title} />;
      ...
      switch (normalizedSource.data_key) {
        case "organizations": {
          ...
          return {
            ...
            get titleJSX() {
              const value = values.link ? (
                <Link title={values.organization_name || AFFINITY_LINK} href={values.link} />
              ) : null;
          ...
          switch (normalizedSource.values_type) {
            case AffinitySourceType.AffinityIntroductionsMadeBy: {
              return {
                ...
                titleString: normalizedSource.values.person_making_intro_name || "",
                id: normalizedSource.values.id || (`${Math.random()}` as SourceID),
                ...
                get titleJSX() {
                  const value = normalizedSource.values.link ? (
                    <Link href={normalizedSource.values.link} title={AFFINITY_LINK} />
                  ) : null;
            ...
            case AffinitySourceType.AffinityIntroductionsMadeTo: {
              return {
                ...
                titleString: normalizedSource.values.person_receiving_intro_name || "",
                id: normalizedSource.values.id || (`${Math.random()}` as SourceID),
                relevance: normalizedSource.relevance || NaN,
                descriptionJSX: null,
                normalizedSource,
            ...
            default: {
              console.log("Unknown source type. This should have a type to discriminate against.", {
                normalizedSource,
              });
