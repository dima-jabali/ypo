...
export const SearchResult: React.FC<{
	...
	previousExpandedSearchResultRef,
	searchStringRegExp,
	searchResult,
	id,
	setOpenView,
	...
	const handleOnSelect = () => {
		...
		if ("database_name" in searchResult.result) {
			...
			pathToExpand.push(
				EntityType.DATABASE,
				`${searchResult.result.database_name}`,
			);
		...
		// Last level:
		pathToExpand.push(
			searchResult.result.entity_type,
	...
	switch (searchResult.result.entity_type) {
				...
				>
						...
						>
							<span className="flex items-center gap-2">
								...
								<p className="truncate text-base">
									{highlightString(name, searchStringRegExp)}
								</p>
							</span>
					...
					{connType && connName ? (
						...
						>
							...
							<p className="truncate text-sm">
								{highlightString(connName, searchStringRegExp)}
							</p>
						</section>
					) : null}
					...
					{schemaName ? (
						...
						>
							...
							<p className="truncate text-sm">
								{highlightString(schemaName, searchStringRegExp)}
							</p>
						</section>
					) : null}
					...
					{tableName ? (
						...
						>
							...
							<i className="truncate text-sm">
								{highlightString(tableName, searchStringRegExp)}
							</i>
						</section>
					) : null}
				</li>
				...
				>
					...
					{dbName ? (
						...
						>
							...
							<p className="truncate text-sm">
								{highlightString(dbName, searchStringRegExp)}
							</p>
						</section>
					) : null}
				</li>
		...
		case EntityType.TABLE: {
			const tableCatalogName =
				searchResult.result.extra_info &&
				"table_catalog" in searchResult.result.extra_info
