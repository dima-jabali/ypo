...
import { IconType, type FetchSchemaRequestParams, type TableName } from "./helpers/types";
...
export function SchemaTree({ expandedItemsRef, activeItemRef, db }: SchemaTreeProps) {
  const [dbParams, setDbParams] = useState<FetchDatabaseDataParams>();

  const searchResultItemsToExpand = databasesSchemaStore.use.searchResultItemsToExpand();
  ...
  const schema = useMemo(
    () =>
      databasesSchema.find((item) => item.databaseId === db.id && db.type === item.connectionType),
  ...
  if (
    searchResultItemsToExpand[INDEX_OF_LOCAL_INDICATOR_THAT_TREE_SHOULD_EXPAND] ===
