...
import { useDownloadedNotebookId, useSortedNotebookBlocks } from "#/hooks/fetch/use-fetch-notebook";
import { usePatchNotebookBlocks } from "#/hooks/mutation/use-patch-notebook-blocks";
import { BlockObjectType, NotebookActionType, type BlockLabel } from "#/types/notebook";
import { useListWithArrowKeysFocus } from "../../hooks/use-list-with-arrow-keys-focus";
import { COMMAND_POPOVER_STYLE_VARS, DEFAULT_FILTER_RESULTS } from "../../utils/utils";
...
function CommandPopoverContent() {
  ...
  function getNextBlockUuid() {
    ...
    const blockAboveIndex = sortedBlocks.findIndex((block) => block.uuid === anchor.blockAboveUuid);
  ...
  function handleOnSelect(
    ...
    const button = (() => {
      const target =
        selectedButton ||
        (e.target as HTMLButtonElement | HTMLInputElement | HTMLImageElement | undefined);
    ...
    if (e.type !== "pointerup") {
      if (
        e.type === "keydown" &&
        (e as React.KeyboardEvent<HTMLButtonElement> | KeyboardEvent).code !== "Enter"
      ) {
        return;
      }
    }
  ...
  function handleClosePopoverOnBackspace(e: React.KeyboardEvent<HTMLInputElement>) {
  ...
  return (
    ...
    >
      {isStreaming ? (
        <p className="px-2 py-1 text-sm text-primary">Cannot add blocks while streaming</p>
      ) : (
        <>
          <div className="border-white-gradient relative box-border flex gap-2 overflow-x-hidden px-2 py-1 tabular-nums text-primary">
            ...
            <input
              ...
              onChange={(e) => slashStore.setState({ filterRawString: e.target.value })}
          ...
          {filterResults.length === 0 ? (
            ...
            <div
              ...
              ref={listRef}
            >
              {filterResults.map(({ icon, title, blockType, subtype }, index) => (
                <button
