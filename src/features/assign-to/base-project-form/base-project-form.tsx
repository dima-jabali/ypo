...
type Props = PropsWithChildren<{
  ...
  setAssignTo: React.Dispatch<React.SetStateAction<Array<ProjectAssigneeForMultiSelect>>>;
...
export const NO_MORE_ITEMS_TO_SELECT = (
  <p className="flex h-12 w-full items-center justify-center p-2">No more items to select</p>
);
...
export function BaseProjectForm({
  ...
  const showEditProjectDescription = generalContextStore.use.showEditProjectDescription();
  const showEditProjectAssignTo = generalContextStore.use.showEditProjectAssignTo();
  const showEditProjectPriority = generalContextStore.use.showEditProjectPriority();
    ...
    >
      ...
      {tags && showEditProjectTags ? (
        <fieldset className="flex flex-col gap-1">
          ...
          <TagGroup
            footer={<TagCreationModal setSelectedTags={setSelectedTags} allTags={tags} />}
...
>["renderRemovableItem"] = ({ name, color }, index, handleRemoveSelectedValue) => (
  <div /* Selected item container */
    className={`relative box-border flex w-min items-center justify-center overflow-hidden rounded ${getProjectTagColors(
      color,
    )}`}
...
const renderTagItem: TagGroupProps<NotebookTag>["renderItem"] = (item, handleAddSelectedValue) => (
  <div key={item.name}>
    <button
...
const renderPriorytyItem: TagGroupProps<(typeof PRIORITY_OPTIONS)[0]>["renderItem"] = (
  item,
...
const renderStatusItem: TagGroupProps<(typeof STATUS_OPTIONS)[0]>["renderItem"] = (
  item,
  handleAddSelectedValue,
) => (
  <div key={item.name}>
