...
import { Dialog, DialogContent, DialogOverlay, DialogTitle } from "#/components/Dialog";
    ...
    try {
      ...
      if (shouldCreateOrgWithProject) {
        ...
        generalContextStore.setState({
          botConversationId: projectMetadata.metadata.bot_conversation?.id ?? null,
  ...
  return (
    <Dialog onOpenChange={setIsOpen} open>
      ...
      <DialogContent className="max-w-sm md:max-w-sm">
        <form onSubmit={handleCreateOrganization} className="flex flex-col gap-8" ref={formRef}>
          <DialogTitle className="mb-5 text-xl font-bold">Create organization</DialogTitle>
