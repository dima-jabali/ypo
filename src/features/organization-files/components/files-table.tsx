...
import { ContextMenuContent, ContextMenuItem, ContextMenuTrigger } from "#/components/ContextMenu";
...
import { GeneralFileType, type GeneralFile, type GoogleDriveFile } from "#/types/notebook";
...
function FileTableRow({ file }: FileTableRowProps) {
  ...
  const handleRenameInputOnKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
    if (!isRenaming) return;
  ...
  const createdAt = file.created_at
    ...
    : "—";
  const isFolder =
    file.type === GeneralFileType.GOOGLE_DRIVE_FOLDER || file.type === GeneralFileType.FOLDER;
