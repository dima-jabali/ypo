...
import { type MutateGeneralFileRequest, useMutateFile } from "../hooks/mutation/useMutateFile";
...
export const FileDetailsSheet: React.FC = () => {
  ...
  const openedFileDetailsFileId = useOrganizationFilesStore().use.openedFileDetailsSheetOfFileId();
  ...
  const file = (
    isValidNumber(openedFileDetailsFileId)
      ? (fetchOrganizationFiles.results.find(({ id }) => id === openedFileDetailsFileId) ?? null)
      : null
  ...
  const googleDriveUrl = (getIsGoogleDriveFile(file) && file.google_drive_url) || null;
    ...
    <Sheet onOpenChange={handleClose} open>
      <SheetContent className="simple-scrollbar">
        <SheetHeader>
          ...
          <SheetDescription>You can view and edit this file&apos;s metadata here.</SheetDescription>
        </SheetHeader>
        ...
        <section className="flex flex-col gap-6">
          ...
          <fieldset className="flex gap-4 items-center justify-between">
            ...
            <span className="text-base">
              {file.created_at
                ? organizationFilesStore.getState().dateFormatter.format(new Date(file.created_at))
                : "—"}
            </span>
          </fieldset>
          ...
          <fieldset className="flex gap-4 items-center justify-between">
            ...
            <span className="text-base">
              {file.updated_at
                ? organizationFilesStore.getState().dateFormatter.format(new Date(file.updated_at))
                : "—"}
            </span>
          </fieldset>
