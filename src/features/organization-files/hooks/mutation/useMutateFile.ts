import { useMutation, useQueryClient, type MutationObserverOptions } from "@tanstack/react-query";
...
export function useMutateFile() {
  ...
  queryClient.setMutationDefaults(mutationKey, {
    mutationFn: async ({ body, fileId }) => {
      const res = await clientAPI_V1.post<MutateGeneralFileResponse>(`/files/${fileId}`, body);
      ...
      queryClient.setQueryData<GetOrganizationFilesResponse>(
        ...
        (cachedOrganizationFilesPage) => {
          ...
          if (cachedFileIndex === -1) {
            console.log("The updated file is not in the list. No need to update it!", {
              cachedOrganizationFilesPage,
              updatedFileFromResponse,
            });
  ...
  } satisfies MutationObserverOptions<MutateGeneralFileResponse, Error, MutateGeneralFileRequest>);

  return useMutation<MutateGeneralFileResponse, Error, MutateGeneralFileRequest>({
    mutationKey,
  });
}
