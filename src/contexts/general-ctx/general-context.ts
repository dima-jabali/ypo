...
export type GeneralContextData = {
  ...
  setBotConversation: (
    botConversationId: BotConversationId,
    newBotConversation: Updater<BotConversation | undefined, BotConversation | undefined>,
  ) => BotConversation | undefined;
  getBotConversation: (botConversationId: BotConversationId) => BotConversation | undefined;
  setBotPlan: (
    botConversationId: BotConversationId,
    organizationId: OrganizationId,
    notebookId: NotebookId,
  ...
  setSettings(
    organizationId: OrganizationId,
    notebookId: NotebookId,
    newSettings: Updater<SettingsReturnType | undefined, SettingsReturnType | undefined>,
  ): void;
  getBotPlan: (
    botConversationId: BotConversationId,
    organizationId: OrganizationId,
    ...
    subscribeWithSelector<GeneralContextData>(
      (_set, get) =>
        ({
          ...
          getBotConversationMessageListPages(botConversationId: BotConversationId) {
            return queryClient.getQueryData<BotConversationMessageListPageInfiniteResponse>(
              queryKeyFactory.get["bot-conversation-message-list-page"](botConversationId).queryKey,
            );
          },
          ...
          getUser() {
            const { clerkApiToken, isUsingLocalClerk, token } = authStore.getState();
    ...
    ),
    {
      partialize(state) {
        const botConversationId =
          state.botConversationId === (OPTIMISTIC_NEW_NOTEBOOK_ID as unknown as BotConversationId)
            ? null
            : state.botConversationId;
        const batchTableId =
          state.batchTableId === (OPTIMISTIC_NEW_NOTEBOOK_ID as unknown as BatchTableId)
            ? null
            : state.batchTableId;
        const notebookId =
          state.notebookId === OPTIMISTIC_NEW_NOTEBOOK_ID ? null : state.notebookId;
...
export const generalContextStore = createReactSelectors(generalContextStoreBase);
