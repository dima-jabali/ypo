...
import { createISODate, createRequestId, createUUID, isValidNumber, sleep } from "#/helpers/utils";
import type {
  BatchTableId,
  BotConversationId,
  ISODateString,
...
const authenticateActor = fromPromise(
  async ({
    ...
    const messagePayload: WebSocketAuthPayload & WebSocketActionData & ExtraDataForEachMessage = {
      message_payload: {
        [token.type === AuthTokenType.Bearer ? "token" : "api_key"]: token.token,
      },
...
const subscribeToNotebookActor = fromPromise(
  async ({
    ...
    if (
      !(isValidNumber(input.context.botConversationId) && isValidNumber(input.context.notebookId))
    ) {
...
export const websocketStateMachine = setup({
    ...
    "subscribe-to-batch-table": {
      actions: [
        ({ context, self, event }) => {
          ...
          if (context.isSubscribedToBatchTable && isValidNumber(previousBatchTableId)) {
    ...
    "subscribe-to-notebook-and-bot-conversation": {
      actions: [
        ({ context, self, event }) => {
          ...
          if (context.isSubscribedToNotebook && isValidNumber(previousNotebookId)) {
          ...
          if (context.isSubscribedToBotConversation && isValidNumber(previousBotConversationId)) {
            ...
            sendWebSocketMessage(context, unsubscribeBotConversationMessagePayload);
          }
  ...
  states: {
    ...
    authenticating: {
      exit: [log("Authenticated WebSocket."), assign({ isAuthenticated: true })],
