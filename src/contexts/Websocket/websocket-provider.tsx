...
import { generalContextStore, SidebarTab } from "#/contexts/general-ctx/general-context";
...
import { WebsocketEvent, WebSocketAuthStatus, type WebSocketEventData } from "#/types/websocket";
...
const WEBSOCKET_URL = process.env.NEXT_PUBLIC_WEBSOCKET_BACKEND_URL as string | undefined;

if (!WEBSOCKET_URL) {
  throw new Error("process.env.NEXT_PUBLIC_WEBSOCKET_BACKEND_URL is not defined");
}

function WebsocketProviderInner({ children }: PropsWithChildren) {
  const generalStoreBotConversationId = generalContextStore.use.botConversationId();
  ...
  useEffect(() => {
    const isOptimisticNotebook = generalStoreNotebookId === OPTIMISTIC_NEW_NOTEBOOK_ID;
    ...
    if (isValidNumber(generalStoreBotConversationId) && isValidNumber(generalStoreNotebookId)) {
      send({
  ...
  }, [generalStoreBotConversationId, generalStoreNotebookId, batchTableId, send]);
      ...
      try {
        ...
        switch (data.message_type) {
          case WebsocketEvent.CheckAuthResponse: {
            if (data.message_payload.is_authenticated || data.message_payload.is_authorized) {
              if (
          ...
          case WebsocketEvent.SubscribeProjectResponse: {
            const hasSubscribed = data.message_payload.status === WebSocketAuthStatus.Success;
          ...
          case WebsocketEvent.SubscribeBotConversationResponse: {
            ...
            const botConversationId = generalContextStore.getState().botConversationId;
          ...
          case WebsocketEvent.UnsubscribeProjectResponse: {
            const hasUnsubscribed = data.message_payload.status === WebSocketAuthStatus.Success;
          ...
          case WebsocketEvent.SqlAutocompleteResponse: {
            const editorData = allEditorsInfo.get(data.message_payload.editor_model_id);
          ...
          case WebsocketEvent.RelevantQueries: {
            ...
            if (notebook) {
              ...
              const block = notebook.blocks.find(({ uuid }) => uuid === blockUuid);
  ...
  const providerValue = useMemo(
    () => ({
      ...
      tryToSubscribeToFileUpdates(fileId: FileId) {
        console.warn("[UNIMPLEMENTED] Trying to subscribe to file updates", fileId);
...
export function WebsocketProvider({ children }: PropsWithChildren) {
  ...
  return <WebsocketProviderInner key={organizationId}>{children}</WebsocketProviderInner>;
}
