...
import type { BotConversationMessage, BotConversationMessageUuid } from "#/types/chat";
import {
  ColorScheme,
  type Base64File,
  type ISODateString,
...
import type { GeneralFile, NotebookBlock, NotebookBlockUuid, NotebookUuid } from "#/types/notebook";
...
export function convertArrayOfObjectsToCSV(arr: Record<string, string | number>[]) {
  const array: (Record<string, string | number> | string[])[] = [Object.keys(arr[0] || {})];
...
export const createNotebookUuid = () => (globalThis.crypto?.randomUUID() || "") as NotebookUuid;
export const createUUID = () => (globalThis.crypto?.randomUUID() || "") as UUID;
export const createRequestId = () => (globalThis.crypto?.randomUUID() || "") as RequestId;
...
export function dbg(...params: unknown[]) {
  ...
  const caller = callerInfo?.split("/").at(-1)?.replace(")", "") || "<unknown file>";
...
export const isLeftMultipleOfRight = (left: number, right: number) => left % right === 0;
...
export const GET_AWS_FILE_AS_STRING_BINARY_ACTION = "GET_AWS_FILE_AS_STRING_BINARY_ACTION";
export const UPLOAD_FILE_STRING_TO_AWS_ACTION = "UPLOAD_FILE_STRING_TO_AWS_ACTION";
