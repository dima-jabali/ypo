...
import type { BotConversationId, ISODateString, NotebookId, OrganizationId } from "#/types/general";
import { NotebookActionType, type Notebook, type PatchNotebookAction } from "#/types/notebook";
import {
  ResponseProjectActionType,
  type PatchProjectResponseAction,
} from "#/types/post-block-update-types";

function sortByCreatedAt(a: { created_at: ISODateString }, b: { created_at: ISODateString }) {
  ...
  request.updates.forEach((action) => {
    switch (action.action_type) {
        ...
        if (alreadyExistingBlockIndex === -1) {
          ...
          if (blockAboveIndex === -1) {
            if (patchedNotebook.blocks.length === 0) {
              ...
              console.error("Block above uuid not found and the notebook is not empty!", {
                action,
                patchedNotebook,
              });

              throw new Error("Block above uuid not found and the notebook is not empty!");
            }
          } else {
      ...
      case NotebookActionType.DeleteBlock: {
        ...
        const blockIndex = patchedNotebook.blocks.findIndex(({ uuid }) => uuid === blockUuid);

        if (blockIndex === -1) {
          console.log("[request-update] Block not found! Not deleting it.", blockUuid);
  ...
  try {
    ...
    if (params.appendNestedString) {
      if (typeof currentObj[lastKey] !== "string") {
        throw new Error(`Tried to append string but target value isn't a string`);
      }
  ...
  response.updates.forEach((action) => {
    switch (action.action_type) {
      ...
      case ResponseProjectActionType.CreateBotConversationMessage: {
        {
          ...
          if (updateBotConversationId !== patchedNotebook.metadata.bot_conversation?.id) {
        ...
        const alreadyExistingIndex = lastPage?.results.findIndex((conv) => conv.uuid === uuid);

        if (isValidNumber(alreadyExistingIndex) && alreadyExistingIndex !== -1 && lastPage) {
      ...
      case ResponseProjectActionType.UpdateBlock: {
        ...
        const { is_incremental_string_change, value, key } = action.action_payload;
      ...
      case ResponseProjectActionType.UpdateBotConversationMessage: {
        ...
        const patchedMessageIndex = arr.findIndex(({ uuid }) => uuid === msgUuid);
        const patchedMessage = patchedMessageIndex === -1 ? null : { ...arr[patchedMessageIndex]! };
      ...
      case ResponseProjectActionType.UpdateBotConversation: {
        const botConversation = getBotConversation(response.bot_conversation_id);

        if (!botConversation || botConversation.id !== action.action_payload.id) {
          break;
        }
        ...
        setBotConversation(response.bot_conversation_id, patchedBotConversation);
      ...
      case ResponseProjectActionType.CreatePlanStep: {
        const plan = getBotPlan(response.bot_conversation_id, organizationId, response.project_id);
        ...
        const planStepIndex = patchedPlan.sub_tasks.findIndex(({ id }) => id === planStepId);
        ...
        if (planStepIndex === -1) {
          ...
          if (isValidNumber(parentStepId)) {
            ...
            const parent = patchedPlan.sub_tasks.find(({ id }) => id === parentStepId);
          ...
          patchedPlan.sub_tasks[planStepIndex] = action.action_payload.plan_step;
        }

        setBotPlan(response.bot_conversation_id, organizationId, response.project_id, patchedPlan);
      ...
      case ResponseProjectActionType.UpdatePlan: {
        ...
        const planStep = patchedPlan.sub_tasks!.find(({ id }) => id === plan_step_id);
