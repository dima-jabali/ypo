...
type HandleDataPreviewProps = {
  ...
  putSavedDataInTheTable: (data: TableDataAsArray, num_rows: number | null, page: number) => void;
...
export const handleDataPreview = ({
  ...
  if (data && Array.isArray(data)) {
    ...
    putSavedDataInTheTable(dataAsArrayFromMap, num_rows, Math.round(offset / limit + 1));
  }
};
...
export const ZERO_RESULTS_KERNEL_RESULTS: Array<KernelResult> = [
  {
    ...
    value: "",
    reactNode: (
      <article className="flex w-full gap-4 border border-blue-400 p-4" key="no-rows">
