...
function isGoingToFirstNotebookIfItIsEmpty(): boolean {
  ...
  if (isEmpty) {
    console.log("First notebook is empty. Goind to it instead of creating a new chat.");

    handleGoToChat(firstNotebookMetadata.id, firstNotebookMetadata.bot_conversation?.id ?? null);
...
export const NotebookListTab = memo(function NotebookListTab() {
  ...
  async function handleCreateChat(createAnyway?: boolean) {
    ...
    if (isGoingToFirstNotebookIfItIsEmpty() && !createAnyway) {
      toast({
        title: "First notebook is empty. Goind to it instead of creating a new chat.",
      });
