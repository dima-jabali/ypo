...
export const PRIORITY_OPTIONS =
	Object.values(NotebookImportance).map(mapPriority);
...
export const EditNotebookModal = memo(function EditNotebookModal({
	...
	const [priority, setPriority] = useState([
		mapPriority(notebook.priority as NotebookImportance),
	]);
	const [status, setStatus] = useState([
		mapStatus(notebook.status as NotebookStatus),
	]);
	const [assignTo, setAssignTo] = useState(
		mapAssignedTo(notebook.assigned_to ?? []),
	);
	const [selectedTags, setSelectedTags] = useState(
		mapTagIds(notebook.tags, allTags || []),
	);
	...
	async function handleSave() {
		...
		try {
			...
			body.assigned_to = assignTo.map((assignTo) => ({
				is_owner:
					assignTo.is_owner || assignTo.user.id === notebook.created_by.id,
				user: assignTo.user,
			}));
								...
								<Dialog onOpenChange={setIsConfirmOpen} open={isConfirmOpen}>
									...
									<DialogContent>
										...
										<DialogDescription className="text-base text-primary">
											...
											&quot;? If you made changes, save them first, then
											archive.
										</DialogDescription>
