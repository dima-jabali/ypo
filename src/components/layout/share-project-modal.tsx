...
function throwIfNotAtLeastOneAdmin(newPermissions: Array<NotebookPermission>) {
  if (newPermissions.length === 0) {
    throw new Error("At least one admin is required. There are no permissions.");
  }

  let hasOneAdmin = false;

  for (const permission of newPermissions) {
    if (permission.user !== null && permission.permission_level === PermissionLevel.Admin) {
      hasOneAdmin = true;
...
function Content() {
  ...
  const [searchUserResults, setSearchUserResults] = useState(allUsersFromCurrentOrg);
  ...
  const organizationsAlreadySharedWith = new Set(
    notebookMetadata.permissions?.map((permission) => permission.organization?.id),
  );
  const canUserChangePermissions = Boolean(
    notebookMetadata.created_by.id === userId ||
    notebookMetadata.permissions?.find(
      (permission) =>
        permission.user?.id === userId && permission.permission_level === PermissionLevel.Admin,
    ),
  );
  ...
  function getFormData() {
    ...
    const newPermissions = [...structuredClone(notebookMetadata.permissions ?? [])];
      ...
      switch (type) {
        case "level": {
          ...
          if (isValidNumber(permissionId)) {
            const permission = newPermissions.find((permission) => permission.id === permissionId);
  ...
  function handlePerformLocalSearch(normalizedSearchValue: string) {
    ...
    const filteredUsers = allUsersFromCurrentOrg.filter((user) => {
      ...
      return fullName.includes(normalizedSearchValue) || email.includes(normalizedSearchValue);
    });
  ...
  return (
    <PopoverContent align="end" side="right" className="flex-col gap-2 flex max-h-[90vh]">
      <Tabs onValueChange={(tab) => handleChangeTab(tab as Tab)} value={activeTab}>
        ...
        <TabsContent
          ...
          asChild
        >
          <form className="flex flex-col gap-2" onSubmit={preventDefault} ref={formRef}>
            <InputWithIcons
              iconRight={
                <button
            ...
            <h6 className="text-xs text-muted-foreground font-semibold pl-2">Organizations</h6>
              ...
              {allOrganizations.length === 0 ? (
                ...
                allOrganizations?.map((org) => (
                  <OrganizationListItem
                    organizationsAlreadySharedWith={organizationsAlreadySharedWith}
                    updateNotebookMetadata={updateNotebookMetadata}
                    canUserChangePermissions={canUserChangePermissions}
                    getFormData={getFormData}
                    notebookId={notebookId}
...
function PermissionsListItem({
  ...
  const avatarFallback =
    permission.user?.first_name.slice(0, 2) ?? permission.organization?.name.slice(0, 2) ?? "?";
  ...
  return (
    ...
    >
      ...
      <div className="h-full items-center flex gap-1">
        <Select
          disabled={updateNotebookMetadata.isPending || !canUserChangePermissions}
        ...
        >
          {isDeleting ? LOADER : <Trash className="size-3 text-muted-foreground" />}
        </Button>
      </div>
    </li>
  );
