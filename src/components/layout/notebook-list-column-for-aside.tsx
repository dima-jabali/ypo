...
import { handleGoToChat, handlePrefetchChat } from "#/helpers/handle-go-to-chat";
  ...
  return (
    <ul className="flex flex-col @max-[15rem]:opacity-0 transition-[opacity] duration-75 max-h-full simple-scrollbar overflow-x-hidden">
      {notebookMetadataList.map((notebook, index) => {
        if (index === 0) {
          handlePrefetchChat(notebook.id, notebook.bot_conversation?.id ?? null, organizationId);
        }
        ...
        return (
          ...
          >
            <button
              onClick={() => {
                handleGoToChat(notebook.id, notebook.bot_conversation?.id ?? null);
            ...
            >
              ...
              <span className="ml-auto text-xs text-muted-foreground sr-only">{notebook.id}</span>
            </button>

            <EditNotebookModal key={notebook.uuid} notebook={notebook} allTags={tags} chatMode />
          </li>
        );
      })}
