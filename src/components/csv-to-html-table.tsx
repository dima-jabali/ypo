...
function LinkCell({ url }: { url: string }) {
  return (
    <a className="link hover:underline" rel="noopener noreferrer" target="_blank" href={url}>
      {url}
    </a>
  );
}
...
function CsvToHtmlTable_({ enableColumnResizing = true, className, csv }: Props) {
  const parseCsvQuery = useQuery({
    ...
    async queryFn() {
      const promise = new Promise<{
        ...
        try {
          const parsedCSVAsColumns = parse(csv, PARSE_CSV_OPTIONS) as unknown as ParsedCsv;
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  const table = useReactTable({
    data: isParsed ? parseCsvQuery.data.parsedCSVAsColumns : DEFAULT_COLUMNS_AND_DATA,
  ...
  return parseCsvQuery.isPending ? (
    ...
    <div className={cn("rounded-lg overflow-hidden border border-border-smooth", className)}>
...
export const CsvToHtmlTable = memo(function CsvToHtmlTable(
  ...
  return withErrorBoundary ? (
    <DefaultSuspenseAndErrorBoundary failedText="Failed to parse CSV!" fallbackFor="CsvToHtmlTable">
