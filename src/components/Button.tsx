  ...
  "inline-flex items-center justify-center rounded-md text-sm font-medium aria-disabled:brightness-75 disabled:brightness-70 gap-3 flex-none aria-disabled:cursor-not-allowed",
  {
    variants: {
      variant: {
        ...
        outline: "border border-border-smooth  bg-popover hover:bg-accent hover:text-white",
...
const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    return (
      ...
      >
        {isLoading ? (
          <Loader className={classNames("size-4 flex-none border-t-white", loaderClassNames)} />
        ) : (
          icon
        )}
