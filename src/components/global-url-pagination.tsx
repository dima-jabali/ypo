...
export const GlobalUrlPagination = memo(function GlobalUrlPagination({
  ...
  const isNewPageValid = (page: number) => {
    return Number.isFinite(page) && page >= 1 && page <= totalNumberOfPages && page !== 0;
  };
  ...
  const handleCurrentPageInputEnterPressed = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
  ...
  const handleCurrentPageInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleCurrentPageLimitInputEnterPressed = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      const input = e.target as HTMLInputElement;
  ...
  const handleCurrentPageLimitInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { valueAsNumber } = e.target;
    ...
    >
      ...
      <section className="flex gap-6 items-center">
        ...
        <div className="inline-flex h-8 gap-4 text-sm font-bold leading-8">
          ...
          <button
            className="flex h-8 items-center justify-center rounded-lg border-2 border-slate-400/80 px-3 leading-tight text-primary button-hover disabled:opacity-50 disabled:cursor-not-allowed"
            disabled={actualCurrentPage === totalNumberOfPages || totalNumberOfPages === 0}
