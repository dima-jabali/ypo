...
import { useAllDataframes, useDownloadedNotebookId } from "#/hooks/fetch/use-fetch-notebook";
...
export function DatabaseConnectionsModal({
  ...
  const [isOpen, setIsOpen] = useReducer((_prev: boolean, next: boolean): boolean => {
    if (next === true) {
    ...
    const updates: Array<PatchNotebookAction> = [
      {
        ...
        action_info: {
          value: isDataframe ? SqlBlockSourceType.Dataframes : SqlBlockSourceType.Integration,
  ...
  return (
    <Popover onOpenChange={setIsOpen} open={isOpen}>
      <PopoverTrigger
        ...
        disabled={disabled}
      >
        <span title={selectedDatabaseOrDataframe?.name || undefined} className="truncate">
          {selectedDatabaseOrDataframe?.name}
        </span>

        {patchNotebookBlocks.isPending ? (
          <div className="flex h-full items-center justify-center px-2 pt-[2px]">{LOADER}</div>
        ) : (
...
function getDatabaseOrDataframeTypeIcon(dbOrDf: NormalDatabaseConnection | Variable | undefined) {
  if (!dbOrDf) {
    return null;
  }

  if (
    dbOrDf?.type === (SqlBlockSourceType.Dataframes as unknown as NormalDatabaseConnection["type"])
  ) {
