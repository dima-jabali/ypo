...
export type TagGroupProps<T extends TagGroupMinimumInterface> = {
  ...
  renderItem: (item: T, handleAddSelectedValue: (item: T) => void) => React.ReactNode;
...
export const TagGroup = <T extends TagGroupMinimumInterface>({
  ...
  const [isOpen, setIsOpen] = useReducer((_prev: boolean, nextIsOpen: boolean) => {
  ...
  useLayoutEffect(() => {
    ...
    for (const item of allValues) {
      if (selectedValues.some((selectedValue) => item.id === selectedValue.id)) continue;

      if (searchStringTrimmed) {
        // There's a search string, Show all values that match the search except those already selected.
        if (item.name?.toLocaleLowerCase().trim().includes(searchStringTrimmed)) {
          results.push(item);
        }
      } else {
    ...
    >
      ...
      >
        ...
        {withSearch && !disabled ? (
          ...
          <p className="ml-2 flex min-h-8 items-center justify-center">{placeholder}</p>
        )}
      </div>
              ...
              >
                <div className="flex h-4/5 w-full flex-col simple-scrollbar">
                  <div className="simple-scrollbar rounded min-h-8">
                    {searchResults.length > 0 ? searchResults.map(RenderItem) : noMoreItemsToSelect}
                  </div>
                </div>
