    ...
    onCancelInput: (cause) => {
      if (cause !== "backspace") {
        const at = insertPoint?.current
          ? {
              at: insertPoint.current,
            }
          : undefined;

        editor.tf.insertText(trigger + value, at);
      }
...
const comboboxItemVariants = cva(
  "relative mx-1 flex h-[28px] items-center rounded-sm px-2 text-sm text-foreground outline-none select-none [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
  {
    ...
    variants: {
      interactive: {
        false: "",
        true: "cursor-pointer transition-none hover:bg-accent hover:text-accent-foreground data-[active-item=true]:bg-accent data-[active-item=true]:text-accent-foreground",
      },
    },
  },
);
