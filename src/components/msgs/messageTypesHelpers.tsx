...
export async function handleCopyTextToClipboard(
  text: string,
  setWasCopiedSuccessfully: React.Dispatch<React.SetStateAction<boolean | undefined>>,
) {
  try {
...
export function makeExtraInfoListItem(key: string, value: unknown): React.ReactNode {
  ...
  switch (typeof value) {
    ...
    case "object": {
      ...
      if (Array.isArray(value)) {
        ...
        return Object.entries(value).map(([k, v]) => makeExtraInfoListItem(k, v));
      }
    }
...
export function getJsonInfoOfToolResponseJson(message: BotConversationMessage): React.ReactNode {
  if (!message.json || typeof message.json !== "object") {
    return null;
  }

  const jsxs = Object.entries(message.json).map(([k, v]) => makeExtraInfoListItem(k, v));
