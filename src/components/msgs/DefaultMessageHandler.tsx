...
export const DefaultMessageHandler = memo(function DefaultMessageHandler({ msg }: Props) {
  ...
  const hasFailedToSendMessage = msg.message_status === BotConversationMessageStatus.Error;
  const isBot = msg.sender.sender_type !== BotConversationMessageSenderType.User;
  const isMessageSent = msg.message_status === BotConversationMessageStatus.Complete;
  const isSendingMessage = !isMessageSent && !hasFailedToSendMessage;

  return (
    <MessageWrapper title="Default Message Handler" data-default-message-handler data-id={msg.id}>
