...
import { useEditBotPlan, type EditBotPlanRequestProps } from "#/hooks/mutation/use-edit-bot-plan";
import { PlanApprovalStatus, type BotPlanStepId, type PlanStep } from "#/types/chat";
...
function hasAnyActivePlan(tasks: PlanStep[]) {
  for (const task of tasks) {
    ...
    if (task.sub_tasks && task.sub_tasks.length > 0 && hasAnyActivePlan(task.sub_tasks)) {
      return true;
    }
  }
...
function PlanMessageInner() {
  ...
  const [isPlanMessageOpen, setIsPlanMessageOpen] = useReducer((prev: boolean, next: boolean) => {
  ...
  function handleApprovePlan() {
    ...
    const sub_tasks: EditBotPlanRequestProps["body"]["sub_tasks"] = tasksRef.current
      .filter(isPlanStepValid)
      .map((item) => ({
        ...
        <div className="flex w-full gap-10 justify-between items-center">
          ...
          <div className="flex gap-4 items-center">
            {isPendingUserApproval || isEditing ? null : (
              <Popover>
                <PopoverTrigger className="p-1 rounded-full button-hover" title="More options">
            ...
            {isEditing && !isPendingUserApproval ? (
              <>
                ...
                {isEditing && !isAnyTaskActive ? (
                  <Button variant={ButtonVariant.GHOST} onClick={handleAddTask} title="Add task">
