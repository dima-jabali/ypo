...
function getTaskRef(tasks: PlanStep[], id: number): { task: PlanStep | null; index: number } {
  let index = 0;
...
export function Task({ taskId }: TaskProps) {
  ...
  const isTaskCompleted = (() => {
    ...
    if (!parentTask) {
      indexOfTaskThatHasIsCurrentSetToTrue = tasksRef.current.findIndex((t) => t.is_current_task);
    } else {
    ...
    >
      ...
      <footer className="mt-3 flex flex-col w-full gap-3">
        ...
        <div>
          {hasNestedTasks(task) ? (
            <details open>
              <summary className="cursor-pointer onfocus:underline text-xs">Nested tasks</summary>
