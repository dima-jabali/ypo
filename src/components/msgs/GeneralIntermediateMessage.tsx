...
import { type BotConversationMessage, BotConversationMessageStatus } from "#/types/chat";
...
const makeExtraInfoListItem = (key: string, value: unknown): React.ReactNode => {
...
const getJsonInfoOfToolSelectionResponseJson = (message: BotConversationMessage) => {
  ...
  if (isPlainObject(message.json)) {
    const jsonJsxs = Object.entries(message.json).map(([key, value]) => {
      const hasAnswer = Reflect.get(value, "does_context_contain_answer") === true;
      ...
      return (
        <div key={key}>
          {isLink ? (
            <a className="link hover:underline block break-all" target="_blank" href={key}>
              • {key}
            </a>
          ) : (
          ...
          <div className="ml-4">
            {hasAnswer ? (
              ...
              <span className="text-xs italic">No answer from this source.</span>
            )}
          </div>
        </div>
      );
