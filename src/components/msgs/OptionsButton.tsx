...
async function handleCopyMarkdown(
  ...
  message: BotConversationMessage,
  setHasCopiedMarkdownSuccessfully: React.Dispatch<React.SetStateAction<boolean | undefined>>,
) {
  const markdownText = text ?? message.text ?? "";

  const tranformedText = removeSourceCitations(markdownText);

  await handleCopyTextToClipboard(tranformedText, setHasCopiedMarkdownSuccessfully);
}
...
export const OptionsButtons = memo(function OptionsButtons({ message, text }: Props) {
  ...
  return (
    <footer className="flex w-full items-center justify-start gap-1 text-primary mb-6">
      <button
        onClick={() => handleCopyMarkdown(text, message, setHasCopiedMarkdownSuccessfully)}
