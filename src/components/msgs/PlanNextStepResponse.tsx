...
export const PlanNextStepResponse = memo(function PlanNextStepResponse({
	msg,
}: Props) {
	...
	return (
		<MessageWrapper
			...
			title="Plan Next Step Response"
			data-plan-next-step-response
			data-id={msg.id}
		>
			{showIntermediateMessage ? (
				<details
					...
					<summary className="z-10 @[450px]:ml-[25%] w-fit flex px-0 select-none cursor-pointer">
						...
						>
							...
							<i className="whitespace-nowrap">
								{msg.toggle_text || getMatchingReasoningText(msg)}
							</i>
...
PlanNextStepResponse.whyDidYouRender = true;
