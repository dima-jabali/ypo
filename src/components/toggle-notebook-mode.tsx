...
export function ToggleNotebookMode() {
  ...
  return (
    <Tooltip>
      <TooltipTrigger
        className="flex bg-notebook items-center justify-center border border-border-smooth rounded-l-full button-hover size-6 @3xl:size-7"
        onClick={isChatMode ? handleGoToNotebookMode : handleGoToChatMode}
        title={title}
      >
        <Icon className="size-3 @3xl:size-3.5 stroke-1 text-muted-foreground" />
      </TooltipTrigger>
