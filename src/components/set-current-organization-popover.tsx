...
import { fileToBase64, getIsRunningInIframe, isDev, noop, preventDefault } from "#/helpers/utils";
import {
...
import { useUploadFileAsBase64ToAws } from "#/hooks/mutation/use-upload-file-as-base64-to-aws";
...
import { Check, ChevronDownIcon, FileImage, Pencil, PlusIcon } from "lucide-react";
...
function scrollActiveItemIntoView(ref: HTMLButtonElement | null, isActive: boolean) {
  if (ref && isActive) {
...
export function SetCurrentOrganizationPopover() {
  ...
  const showManageUsersToAdminsInIframe = generalContextStore.use.showManageUsersToAdminsInIframe();
  const showManageUsersToUsersInIframe = generalContextStore.use.showManageUsersToUsersInIframe();
  const showManageUsersToUsersInBb = generalContextStore.use.showManageUsersToUsersInBb();
      ...
      <PopoverTrigger>
        <Tooltip>
          ...
          >
            ...
            <p className="truncate whitespace-nowrap">{currentOrganization.name}</p>
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      <span className="text-xs text-muted" title="Organization's ID">
                        ({org.id})
                      </span>
                    </button>

                    <div className="flex items-center gap-0">
                      {isUserAdminOnThisOrg ? <MutateOrganizationDialog org={org} /> : null}
            ...
            <div className="flex gap-1 border-t border-t-border-smooth pt-1 empty:hidden">
              {isInsideIframe ? (
                <>
                  ...
                  {(isUserRegular && showCreateNewOrganizationToUsersInIframe) ||
                  (showCreateNewOrganizationToAdminsInIframe && isUserAdmin) ? (
                    ...
                    >
                      ...
                      <span className="whitespace-nowrap">Create new organization</span>
                    </button>
                  ) : null}
                </>
              ) : (
                <>
                  {(showManageUsersToUsersInBb && isUserRegular) || isUserAdmin ? (
                    <>
                      <ManageUsersModal key={currentOrganization.id} />
                  ...
                  {(isUserRegular && showCreateNewOrganizationToUsersInBb) || isUserAdmin ? (
                    <button
                      className="flex w-full gap-2 items-center justify-center rounded-[5px] p-2 button-hover text-xs"
                      onPointerUp={() => setIsCreateOrgModalOpen(true)}
                      title="Create new organization"
...
function MutateOrganizationDialogContent({
  ...
  const uploadToAws = useUploadFileAsBase64ToAws();
  ...
  const [
    ...
    initialFiles:
      awsBase64LogoFileQuery.data && typeof awsBase64LogoFileQuery.data === "string"
        ? [
            {
              type: awsBase64LogoFileQuery.data.slice(5, awsBase64LogoFileQuery.data.indexOf(";")),
  ...
  const [
    ...
    initialFiles:
      awsBase64WhitelabelFileQuery.data && typeof awsBase64WhitelabelFileQuery.data === "string"
        ? [
            {
  ...
  async function handleMutateOrganization() {
    function invalidateAwsImg(awsBucket: AwsBucket, awsKey: AwsKey) {
      queryClient
        .invalidateQueries({
          queryKey: [...queryKeyFactory.get["aws-base64-file"].queryKey, awsBucket, awsKey],
        })
        .catch(noop);
    }
    ...
    try {
      ...
      const isExistingLogoFile = logoFile && logoFile.id === `existing-logo-${org.id}`;
  ...
  return (
    <DialogContent className="max-h-[90vh] simple-scrollbar">
      ...
      <fieldset>
        ...
        <Input
          onChange={(e) => (changesRef.current.whitelabel_name = e.target.value)}
      ...
      <fieldset className="flex items-center justify-center gap-2 w-fit">
        <Checkbox
          onCheckedChange={(checked) => {
            changesRef.current.use_whitelabel_image = checked.valueOf() as boolean;
          }}
      ...
      <DialogFooter>
        <Button
          isLoading={
            mutateOrganization.isPending || deleteAwsFile.isPending || uploadToAws.isPending
          }
          onClick={handleMutateOrganization}
          variant={ButtonVariant.SUCCESS}
        >
          Sav
          {mutateOrganization.isPending || uploadToAws.isPending || deleteAwsFile.isPending
            ? "ing..."
            : "e"}
        </Button>
      </DialogFooter>
