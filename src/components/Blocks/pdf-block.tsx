...
function PdfBlockWithContexts({ pdfBlock }: Props) {
  ...
  const pdfFileQuery = useFetchPdfFileById(isValidNumber(pdfMetadata?.id), pdfMetadata?.id);
  ...
  const isReadonly = isStreaming || uploadPdfToNotebookBlock.isPending || indexPdf.isPending;
  const fileName = pdfMetadata?.file_name ?? "";

  function handleUploadFile(file: File) {
    if (isReadonly || uploadPdfToNotebookBlock.isPending || indexPdf.isPending) return;
        ...
        >
          <label className="flex cursor-pointer flex-col items-center justify-center p-4 text-center md:p-8">
            ...
            <p className="mx-auto mt-3 max-w-xs text-primary">
              Click to <span className="font-bold">Upload your PDF file</span> or drag and drop your
              file here
            </p>
