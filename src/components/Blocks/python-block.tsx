...
export const PythonBlock = memo(function PythonBlockWithProviders(props: Props) {
  return (
    <BlockStoreProvider
      extraInitialParams={{
...
function PythonBlockWithContexts({ pythonBlock }: Props) {
  ...
  const commandTextareaValueRef = useRef(pythonBlock.custom_block_info?.command ?? "");
  ...
  function handleAskAiOnMetaEnter(event: React.KeyboardEvent<HTMLTextAreaElement>) {
  ...
  const isDataPreviewStale = pythonBlock.custom_block_info?.is_data_preview_stale || false;
      ...
      <section className="flex flex-col gap-0 border rounded-lg overflow-hidden border-border-smooth">
        ...
        {isNotebookMode ? (
          <div className="flex items-center w-full">
            ...
            >
              {askToGeneratePythonCode.isPending ? LOADER : <Forward className="size-4" />}
            </button>
          </div>
        ) : null}
