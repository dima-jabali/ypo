...
import { useDownloadedNotebookId, useVariablesToSelect } from "#/hooks/fetch/use-fetch-notebook";
...
export const TableBlock = memo(function TableBlock(props: Props) {
  return (
    <BlockStoreProvider
      extraInitialParams={{
        blockFilterAndSort: props.tableBlock.custom_block_info?.filters || DEFAULT_FILTERS,
...
function TableBlockRoot({ tableBlock }: Props) {
  ...
  const isBlockRunning = patchNotebookBlocks.isPending || runTableBlock.isPending;
    ...
    >
      <header className="flex justify-between items-center">
        <Popover onOpenChange={setIsOpen} open={isOpen}>
          ...
          >
            {isBlockRunning ? LOADER : <AtSign className="size-3 flex-none text-accent" />}

            <i>{selectedVariableName ? selectedVariableName : "Select variable"}</i>
          </PopoverTrigger>
                  ...
                  <CommandGroup>
                    {variablesToSelect.map((variable) => (
                      ...
                      >
                        ...
                        <span className="truncate whitespace-nowrap">{variable.name}</span>
                      </CommandItem>
                    ))}
                  </CommandGroup>
                </CommandList>
        ...
        <RunArrow showLoader={isBlockRunning} disabled={isBlockRunning} onClick={handleRunBlock} />
      </header>
