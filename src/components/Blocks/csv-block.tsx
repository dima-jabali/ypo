...
export const CSVBlock = memo(function CSVBlock({ csvBlock }: { csvBlock: BlockCsv }) {
  ...
  const shouldShowTable = totalNumberOfRows !== null && !isBlockRunning && !uploadCsv.isPending;
  const canRunBlock =
    (shouldShowTable || hasCsvFile) && !uploadCsv.isPending && !runCsv.isPending && !isStreaming;
        ...
        >
          <label className="flex cursor-pointer flex-col items-center justify-center p-4 text-center md:p-8">
            ...
            <p className="mx-auto mt-3 max-w-xs text-primary">
              Click to <span className="font-bold">Upload your CSV file</span> or drag and drop your
              file here
            </p>
