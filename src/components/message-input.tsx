...
import { type PlateEditor, useEditorSelector, usePlateEditor } from "platejs/react";
...
export const MessageInput = memo(function MessageInput() {
  ...
  const isOptimisticBotConversation =
    botConversationId === (OPTIMISTIC_NEW_NOTEBOOK_ID as unknown as BotConversationId);
  ...
  const isDefaultToolsSelected =
    currentOrganization.default_chat_tools && toolsToUse
      ? currentOrganization.default_chat_tools.length === toolsToUse.length &&
        toolsToUse.every((tool) => currentOrganization.default_chat_tools?.includes(tool))
      : false;
  ...
  function scrollToBottom() {
    if (scrollContainer) {
      requestAnimationFrame(() => {
        if (scrollContainer) {
          Reflect.set(scrollContainer, "scrollTop", scrollContainer.scrollHeight);
        }
      });
    }
  }
  ...
  async function handleSendMessage(): Promise<void> {
    ...
    if (isOptimisticBotConversation) {
      toast({
        title: "Try again in a few seconds. If the problem persists, please refresh the page.",
        description: "Can't send message because the chat is still being created.",
    ...
    if (isStreaming) {
      toast({
        title: "Please wait for the bot to finish processing the previous message.",
      });
  ...
  function handleStopStreaming() {
    ...
    const messagePayload: WebSocketStopGenerationPayload & ExtraDataForEachMessage = {
  ...
  async function handleShowFilesPreviewOnMessageInput(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setFiles((prev) => prev.concat(files.filter((file) => !file.type.startsWith("image"))));
  }
            ...
            {files.length > 0 ? (
              <div className="flex simple-scrollbar max-h-96 min-h-10 scrollbar-stable scrollbar-both-edges gap-4">
                {files.map((file, index) => {
                  ...
                  const previewUrl: string | undefined = isImage && Reflect.get(file, "previewUrl");
                  ...
                  if (isImage && previewUrl) {
                    return (
                      ...
                      >
                        ...
                        <img className="rounded-sm" src={previewUrl} alt="Preview" />
                      </div>
                    );
                  }
