import {
	KeyCode,
	KeyMod,
	type editor as MonacoEditorType,
	type Position,
} from "monaco-editor";
	...
	useEffect(
		function setupAutocomplete() {
			...
			function setupEditorListeners(
				...
				let lastWordAtPosition:
					| MonacoEditorType.IWordAtPosition
					| null
					| undefined;
				...
				const onKeyDownListener = genericEditor.onKeyDown((e) => {
					...
					const isCursorOnSamePosition =
						lastCursorPosition && position.equals(lastCursorPosition);
					...
					if (wordAtPosition) {
						...
						const hasEncounteredStopWordChar =
							STOPWORD_REGEX.test(wordUntilCursor);
						...
						const shouldSkipAutocompleteTrigger =
							!hasEncounteredStopWordChar &&
							isCursorWalkingOnSameWord &&
							alreadyHasAutocomplete;
				...
				const onDidFocusListener = genericEditor.onDidFocusEditorText(() => {
					...
					if (
						!(
							editorData &&
							editorData.lastCursorPosition &&
							currentCursorPosition
						)
					)
						return;

					const arePositionsEqual = currentCursorPosition.equals(
						editorData.lastCursorPosition,
					);
			...
			if (monacoEditor) {
				...
				removeInfos.push(
					setupNeededValuesForAutoCompletionSuggestions(monacoEditor),
				);
			}
			// if (monacoDiffEditor) {
