...
export function StringFilterCombobox({
  ...
  function handleOnChangeFilterString(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    startTransition(() => {
      ...
      if (filterString) {
        setSearchResults(
          allValuesToFilter.filter((value) => value.toLowerCase().includes(filterString)),
        );
      } else {
      ...
      >
        ...
        <ul className="flex flex-col">
            ...
            return (
              <li
                ...
                onClick={() => {
                  startTransition(() => {
                    if (isSelected) {
                      setSelectedValuesToFilter((prev) => prev.filter((v) => v !== value));
                    } else {
          ...
          {searchResults.length === 0 ? (
            <p className="flex items-center justify-center w-full h-8">No results</p>
          ) : null}
        </ul>
        ...
        {selectedValuesToFilter.length > 0 ? (
          <>
            ...
            <button
              ...
              onClick={() => startTransition(() => setSelectedValuesToFilter([]))}
              type="button"
            >
              Clear filters
            </button>
