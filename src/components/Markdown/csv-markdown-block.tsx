...
function CsvMarkdownBlock_({ csv = "" }: CsvMarkdownBlockProps) {
  ...
  const timerToConsiderCsvReceivedRef = useRef<ReturnType<typeof setTimeout>>(undefined);
  ...
  return isCsvComplete ? (
    <CsvToHtmlTable className="simple-scrollbar self-auto" withErrorBoundary={false} csv={csv} />
  ) : (
    <div className="flex gap-2 items-center">
      <Loader className="border-t-muted-foreground size-3.5" />

      <span className="inline-flex text-sm text-muted-foreground">Receiving CSV</span>
    </div>
  );
}

export const CsvMarkdownBlock = memo(function CsvMarkdownBlock(props: CsvMarkdownBlockProps) {
      ...
      <QueryErrorResetBoundary>
        {({ reset }) => (
          <ErrorBoundary
            ...
            fallback={<LoadError csv={props.csv ?? ""} prevCsv={prevCsv} error={error} />}
            onReset={reset}
          >
...
function FallbackLoader() {
  return (
    <div className="w-full h-full flex items-center justify-center" title="Loading...">
      {LOADER}
    </div>
  );
}
