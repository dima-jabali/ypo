import { anyOf, createRegExp, exactly, global, maybe, whitespace } from "magic-regexp";
...
export const SOURCE_CITATIONS_REGEX = /\[(?:\s*|\**)(?:SOURCE:|WEBSITE:)(.*?)(?:\**)\]/g;
...
export function preprocessSourceCitations(content: string) {
  ...
  const processedContent = content.replaceAll(SOURCE_CITATIONS_REGEX, (_, source: string) => {
...
export function normalizeLatexMath(source: string) {
  ...
  return source.replace(LATEX_MATH_REGEX, (_, content) => `$$\n${content.trim()}\n$$`);
}
