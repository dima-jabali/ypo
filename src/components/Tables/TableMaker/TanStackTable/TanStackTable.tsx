...
import { flexRender, type Table as TanStackTableType } from "@tanstack/react-table";
...
export const TanStackTable: React.FC<{
  ...
  return (
    ...
    <div className={canScroll ? "simple-scrollbar flex h-full w-full max-w-full grow" : undefined}>
      <table
                ...
                return (
                  ...
                  >
                    <button
                      ...
                      onClick={
                        isLocalIndexColumn ? undefined : header.column.getToggleSortingHandler()
                      }
                    ...
                    >
                      ...
                      {isLocalIndexColumn || !canResize
                        ? null
                        : SORT_ICONS[header.column.getIsSorted() as keyof typeof SORT_ICONS]}
                    </button>
          ...
          {table.getRowModel().rows.map((row) => (
            <tr className="odd:bg-alt-row" key={row.id}>
              {row.getVisibleCells().map((cell) => {
                ...
                const jsx = flexRender(cell.column.columnDef.cell, cell.getContext());

                const hasNestedData = typeof renderValue === "object" && renderValue !== null;

                return (
                  <td
                    data-is-index-column={cell.column.columnDef.id === DATA_ID_KEY}
                    title={`${hasNestedData ? "Json Viewer" : renderValue}`}
                    className="data-[is-index-column=true]:text-center px-2"
                    style={{ width: `${cell.column.getSize()}px` }}
                    key={cell.id}
                  >
                    {hasNestedData ? <JsonViewer json={renderValue as object | any[]} /> : jsx}
                  </td>
                );
              })}
            </tr>
