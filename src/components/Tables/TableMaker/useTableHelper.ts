...
export const useTableHelper = (
  ...
  const [store] = useState(() => {
    const store = createStore<TableHelperStore>((set, get) => ({
      numberOfRowsPerPage: initialNumberOfRowsPerPage > 1 ? initialNumberOfRowsPerPage : 10,
      ...
      paginate: async ({
        ...
        const haveFiltersChanged = !isEqual(prevBlockFilterAndSortRef.current, filters);
        ...
        const needToFetch =
          ...
          !hasItemsWithIdsBetween(tableMapStorage, smaller, greater, totalNumberOfRows);
        ...
        try {
          ...
          if (res.action_output.error === "") {
            ...
            setTableArrayStorage(
              haveFiltersChanged ? jsonData : jsonData.concat(tableArrayStorage),
            );
          } else {
      ...
      putSavedDataInTheTable: (
        ...
        tableStartIndexRef.current = (initialPage - 1) * get().numberOfRowsPerPage;

        const tableDataForReducer: TableDataForReducer = tableMapStorageAsArray.map(
          (item) => item[1],
        );
      ...
      setNumberOfRowsPerPage: (next: number) => set({ numberOfRowsPerPage: next }),
