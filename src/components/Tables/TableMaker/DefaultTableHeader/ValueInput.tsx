...
import { lazy, type SetStateAction, Suspense, useEffect, useMemo, useState } from "react";
...
import { type ChildFilter, FilterType } from "#/components/Tables/TableMaker/filters/utilityTypes";
...
export const ValueInput: React.FC<Props> = ({ childFilter }) => {
  ...
  const value = (() => {
    if (typeof childFilter.value === "boolean" || typeof childFilter.value === "object") {
  ...
  switch (childFilter.column.type) {
    case FilterType.timedelta:
      return (
        <div className="flex items-centermin-h-[31px]">
          <Popover onOpenChange={setIsFirstDatePickerOpen} open={isFirstDatePickerOpen}>
            <div className="border-table-separator bg-input-bg text-font-color flex flex-col gap-1 rounded-sm border p-1 tabular-nums">
              <div className="flex h-[31px] w-full items-center justify-between gap-2 pr-1">
                <p className="w-full">
                  {firstDateSelected ? firstDateSelected.toLocaleDateString() : "From"}
                </p>
          ...
          &nbsp;-&nbsp;
          <Popover onOpenChange={setIsSecondDatePickerOpen} open={isSecondDatePickerOpen}>
            <div className="border-table-separator bg-input-bg text-font-color flex flex-col gap-1 rounded-sm border p-1 tabular-nums">
              <div className="flex h-[31px] w-full items-center justify-between gap-1 pr-1">
                <p className="w-full">
                  {secondDateSelected ? secondDateSelected.toLocaleDateString() : "To"}
                </p>
    ...
    case FilterType.datetime64:
      return (
        <Popover onOpenChange={setIsSingleDatePickerOpen} open={isSingleDatePickerOpen}>
          <div className="border-table-separator bg-input-bg text-font-color flex flex-col gap-1 rounded-sm border p-1 tabular-nums">
            <div className="flex h-[31px] w-full items-center justify-between gap-1 pr-1">
              <p className="w-full">
                {singleDateSelected ? singleDateSelected.toLocaleDateString() : "To"}
              </p>
