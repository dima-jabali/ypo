...
import type { ChildFilter, Filter, FilterGroup } from "../../filters/utilityTypes";
...
import { useGroupOfFilters, useTableForceRender } from "../../tableDataContextUtils";
...
export const FiltersToBeApplied: React.FC = () => {
  ...
  const makeGroupOrRow = useCallback((outerFilter: Filter, index: number): React.ReactNode => {
    const makeRow = (childFilter: ChildFilter, index: number): React.ReactNode => {
      indexRef.current++;
    ...
    const makeGroup = (filterGroup: FilterGroup, index: number): React.ReactNode => {
      depthRef.current++;
    ...
    return isAParent(outerFilter) ? makeGroup(outerFilter, index) : makeRow(outerFilter, index);
  }, []);
