...
export function useFetchAllDatabaseConnections<
...
export function useClickupConnection() {
	...
	const select = useCallback(
		(data: AllDatabaseConnections) => {
			return data.clickUpConnections.find(
...
export function useGoogleDriveConnection() {
	...
	const select = useCallback(
		(data: AllDatabaseConnections) => {
			return data.googleDriveDatabases.find(
				(connection) => connection.id === connectionId,
			);
		},
		[connectionId],
...
export function usePlaidConnection() {
	...
	const select = useCallback(
		(data: AllDatabaseConnections) => {
			return data.plaidConnections.find(
				(connection) => connection.id === connectionId,
			);
		},
		[connectionId],
...
export function useSlackConnection() {
	...
	const select = useCallback(
		(data: AllDatabaseConnections) => {
			return data.botDatabaseConnections.find(
				(connection) => connection.id === connectionId,
			);
		},
		[connectionId],
	...
	return useMemo(() => {
		return {
			alreadySelectedChannels:
				slackConnection.channels.filter(isASelectedChannel),
			slackConnection,
		};
	}, [slackConnection]);
}
...
export function useAirtableConnection() {
	...
	const select = useCallback(
		(data: AllDatabaseConnections) => {
			return data.airtableDatabaseConnections.find(
				(connection) => connection.id === connectionId,
			);
		},
		[connectionId],
		...
		(data: AllDatabaseConnections) => {
			return data.normalDatabases.find(
				(connection) =>
					...
					connection.type === DatabaseConnectionType.Postgres,
			);
		},
		[connectionId],
	);
	...
	const select = useCallback(
		(data: AllDatabaseConnections) => {
			return data.normalDatabases.find(
				(connection) =>
					connection.id === connectionId &&
					connection.type === DatabaseConnectionType.Snowflake,
			);
		},
		[connectionId],
	);
