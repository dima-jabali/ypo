...
export function useAddBotConversationMessage() {
	...
	function setIsStreamingTo(
		...
		console.log("Setting isStreaming to", nextValue, {
			requestBotConversationId,
		});

		const { getBotConversation, setBotConversation } =
	...
	>({
		...
		onMutate(variables) {
			const { notebookId, setBotConversationMessageListPages } =
				generalContextStore.getState();
		...
		onError(error, variables, context) {
			...
			const { setBotConversationMessageListPages } =
				generalContextStore.getState();
