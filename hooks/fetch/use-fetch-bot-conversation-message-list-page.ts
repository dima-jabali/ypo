...
import { getReferencedSources, URL_TEXT_SEARCH, WEBSITE_PREFIX } from "#/components/Markdown/pre-processors"
...
export type SelectedBotConversationMessageListPage<SelectedData = BotConversationMessageListPageInfiniteResponse> = (
  data: BotConversationMessageListPageInfiniteResponse,
) => SelectedData

export function useFetchBotConversationMessageListPage<SelectedData = BotConversationMessageListPageInfiniteResponse>(
...
export function selectHasAnyBotConversationMessage(data: BotConversationMessageListPageInfiniteResponse) {
...
export const useHasAnyMessage = (botConversationId: BotConversationId | null) =>
  useFetchBotConversationMessageListPage(botConversationId, selectHasAnyBotConversationMessage).data
...
export function useNormalizedMessages(botConversationId: BotConversationId | null, withNotebookBlocks: boolean) {
...
function selectAllChatSourcesMainValues(
  ...
  const map = new Map<SourceID, SourceMainValues<SourceForUserType, NormalizedSource["values_type"]>>()
        ...
        if (msg.sources && msg.sources.length > 0) {
          ...
          for (const source of normalizedSources) {
            ...
            if (
              ...
              referencedSources.has(mainValues.id.replace(WEBSITE_PREFIX, "").replace(URL_TEXT_SEARCH, "") as SourceID)
            ) {
...
export function useAllChatSourcesMainValues(botConversationId: BotConversationId | null) {
  ...
  return useFetchBotConversationMessageListPage(botConversationId, select).data
}
