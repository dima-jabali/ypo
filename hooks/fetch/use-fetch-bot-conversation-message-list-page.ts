...
export const useHasAnyMessage = () =>
	useFetchBotConversationMessageListPage(selectHasAnyBotConversationMessage)
		.data;
...
export type NormalizedBotConversationMessage = {
	parallelMessages: Map<
		ParallelConversationId,
		Array<BotConversationMessage>
	> | null;
...
export function selectNormalizedMessages(
	...
	let lastNormalizedParallelMessages: NormalizedBotConversationMessage | null =
		null;
	...
	for (let index = 0; index < length; ++index) {
		...
		if (isValidNumber(parallelConversationId)) {
			if (lastNormalizedParallelMessages) {
				const arr = lastNormalizedParallelMessages.parallelMessages!.get(
					parallelConversationId,
				);
				...
				if (arr) {
					...
					if (lastNormalizedParallelMessages.parallelMessages!.size === 2) {
						...
						lastNormalizedParallelMessages.parallelMessages!.set(
							parallelConversationId,
							[msg],
						);
...
export function useNormalizedMessages(withNotebookBlocks: boolean) {
	const toggleHideParallelAnswers =
		generalContextStore.use.toggleHideParallelAnswers();

	const msgs = useFetchBotConversationMessageListPage(
		useCallback(
			(data: BotConversationMessageListPageInfiniteResponse) =>
				selectNormalizedMessages(
					data,
					withNotebookBlocks,
					toggleHideParallelAnswers,
				),
						...
						if (
							...
							referencedSources.has(
								mainValues.id
									...
									.replace(URL_TEXT_SEARCH, "") as SourceID,
							)
						) {
...
export function useAllChatSourcesMainValues() {
	const onlyShowUsedReferences =
		generalContextStore.use.onlyShowUsedReferences();
