...
import { api } from "#/api";
import type { GetBatchTableMetadatasPageRequest } from "#/features/sapien/hooks/get/use-fetch-batch-table-metadatas-page";
...
export const queryKeyFactory = createQueryKeyStore({
	get: {
		...
		"batch-table-data-sources": (organizationId: OrganizationId) => ({
			queryFn: () => api.get["batch-table-data-sources"](organizationId),
		...
		"batch-table-metadata-page": (
			organizationId: OrganizationId,
			queryParams: GetBatchTableMetadatasPageRequest,
		) => ({
			queryFn: (ctx) =>
				api.get["batch-table-metadata-page"](
					ctx.pageParam as GetBatchTableMetadatasPageRequest,
					organizationId,
				),
		...
		"batch-table": (
			...
			queryFn: () => api.get["batch-table"](batchTableId, organizationId),
			queryKey: [organizationId, batchTableId],
		}),
