...
type NormalizedBotConversationMessageWithParallelMessages =
	NormalizedBotConversationMessage & {
		parallelMessages: NonNullable<
			NormalizedBotConversationMessage["parallelMessages"]
		>;
	};
...
function handleParallelMessages(
	...
	const key =
		getMapFirstValue(normalizedBotConversationMessage.parallelMessages)?.[0]
			?.id ?? Math.random();
...
const ParallelMsgDetails = memo(function ParallelMsgDetails({
	children,
...
function handleNormalMessages(
	...
	switch (message.message_type) {
		...
		case BotConversationMessageType.Notebook_Block_Message: {
			msgNode = withNotebookBlocks ? (
				<NotebookBlockMessage msg={message} />
			) : null;
			break;
		}
