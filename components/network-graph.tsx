...
function buildDepthLimitedGraph(
	profiles: Profile[],
	startUserId: number,
	maxDepth: number,
) {
	...
	while (queue.length > 0) {
		...
		if (current.depth < maxDepth && currentProfile.similar_neighbors) {
			currentProfile.similar_neighbors.forEach((neighbor) => {
				...
				// Add edge
				const pairKey = [String(current.id), String(neighbor.id)]
					.sort()
					.join("-");
	...
	return (
		<Card>
			...
			<CardContent className="space-y-3">
				...
				<Button className="w-full" onClick={handleProfileClick}>Visit Profile</Button>
			</CardContent>
		</Card>
	);
}
...
function EdgeInfoCard({
	edge,
	graphData,
}: { edge: any; graphData: { nodes: GraphNode[]; links: GraphLink[] } }) {
	...
	const similarityScore = edge.similarity
	...
	const { data: similarityData, isLoading } = useProfileSimilarity(
		sourceId,
		targetId,
	);
				...
				{similarityData?.similarity_reasons && (
					<div className="space-y-3 pt-2 border-t">
						<p className="text-sm font-semibold text-primary">
							{similarityData.similarity_reasons.title}
						</p>
						...
						{similarityData.similarity_reasons
							.what_you_might_learn_from_each_other && (
							<div className="space-y-1">
								<p className="text-xs font-medium">
									What you might learn from each other
								</p>
								<p className="text-xs text-muted-foreground">
									{
										similarityData.similarity_reasons
						...
						{similarityData.similarity_reasons
							.how_you_might_be_helpful_to_each_other && (
							<div className="space-y-1">
								<p className="text-xs font-medium">
									How you might be helpful to each other
								</p>
								<p className="text-xs text-muted-foreground">
									{
										similarityData.similarity_reasons
...
export function NetworkGraph() {
	...
	const impersonatedProfileId = useStore(
		(state) => state.impersonatedProfileId,
	);
	...
	useEffect(() => {
		function resetFocusOnEsc(e: KeyboardEvent) {
			if (e.key === "Escape") {
				setFocusedElement(null);
			}
		}

		window.addEventListener("keypress", resetFocusOnEsc);

		return () => {
			window.removeEventListener("keypress", resetFocusOnEsc);
		};
	}, []);
			...
			<div className="flex items-center justify-between mb-6">
				<div>
					<h1 className="text-3xl font-bold text-balance">
						My Network{" "}
						{currentUserProfile?.name && `- ${currentUserProfile.name}`}
					</h1>
					<p className="text-muted-foreground mt-1">
						Showing connections up to {selectedDepth} degrees of separation
					</p>
				</div>
				...
				<div className="space-y-4">
					{focusedElement ? (
						...
						<Card className="border-dashed">
							...
							<CardContent>
								<p className="text-xs text-muted-foreground">
									Click or hover over nodes and edges to see details here
								</p>
							</CardContent>
						</Card>
					)}
