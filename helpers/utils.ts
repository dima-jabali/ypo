...
import { MENTION_REGEX } from "#/features/sapien/components/column-options-popover/convert-string-to-plate-value";
import type {
	BatchTableCell,
	BatchTableCellUuid,
	BatchTableColumn,
...
export function stringifyCellValue(
	...
	switch (typeof cellValue) {
		...
		case "string": {
			return cellValue.replaceAll(MENTION_REGEX, (_match, columnId) => {
				const columnIdAsNumber = toNumber(columnId);

				const column = mentionables.find(({ id }) => id === columnIdAsNumber);

				if (column) {
					return `${column.name} (id: ${columnId})`;
				}
		...
		case "function": {
			return stringifyCellValue(cellValue(), isFromFileColumn, mentionables);
		}
		...
		case "object": {
			if (isFromFileColumn) {
				if (Array.isArray(cellValue)) {
					...
					for (const fileFromCell of cellValue as Array<GeneralFile>) {
						const fileName = fileFromCell.file_name || "";
			...
			if (Array.isArray(cellValue)) {
				return `[${cellValue.map((v) => stringifyCellValue(v, isFromFileColumn, mentionables)).join(", ")}]`;
			}

			try {
				return JSON.stringify(cellValue, null, 2);
			} catch (error) {
				console.log("Failed to stringify cell value!", { cellValue, error });
