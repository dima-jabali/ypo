...
function Members() {
	...
	const uniquePositions = useMemo(
		() =>
			Array.from(
				new Set(members.map((m) => m.position).filter(Boolean)),
			).sort(),
		[members],
	);
	const uniqueCompanyNames = useMemo(
		() =>
			Array.from(
				new Set(members.map((m) => m.current_company_name).filter(Boolean)),
			).sort(),
		[members],
	);
	const uniqueCompanyIndustries = useMemo(
		() =>
			Array.from(
				new Set(members.map((m) => m.current_company_industry).filter(Boolean)),
			).sort(),
		[members],
	);
			...
			<div className="flex flex-col gap-4">
				<div>
					...
					<p className="text-muted-foreground">
						Search and connect with {data?.num_results || members.length}+ YPO
						members worldwide
					</p>
				</div>
						...
						<div className="space-y-4">
							...
							<div className="grid grid-cols-1 md:grid-cols-4 gap-3">
								<Select
									...
									onValueChange={(value) =>
										setIndustryFilter(value || undefined)
									}
								>
									<SelectTrigger>
								...
								<Select
									value={chapterFilter || ""}
									onValueChange={(value) =>
										setChapterFilter(value || undefined)
									}
								>
									<SelectTrigger>
							...
							<div className="grid grid-cols-1 md:grid-cols-4 gap-3">
								<Select
									...
									onValueChange={(value) =>
										setCompanyIndustryFilter(value || undefined)
									}
								>
									<SelectTrigger>
										<Building2 className="h-4 w-4 mr-2" />
								...
								<Select
									...
									onValueChange={(value) =>
										setPositionFilter(value || undefined)
									}
								>
									<SelectTrigger>
								...
								<Select
									...
									onValueChange={(value) =>
										setCompanyNameFilter(value || undefined)
									}
								>
									<SelectTrigger>
										<Building2 className="h-4 w-4 mr-2" />
								...
								<Select
									...
									onValueChange={(value) =>
										setLocationFilter(value || undefined)
									}
								>
									<SelectTrigger>
										<MapPin className="h-4 w-4 mr-2" />
							...
							<div className="grid grid-cols-1 md:grid-cols-4 gap-3">
								<Select
									...
									onValueChange={(value) =>
										setLanguagesFilter(value || undefined)
									}
								>
									<SelectTrigger>
								...
								<Select
									value={hobbiesFilter || ""}
									onValueChange={(value) =>
										setHobbiesFilter(value || undefined)
									}
								>
									<SelectTrigger>
								...
								<Select
									...
									onValueChange={(value) =>
										setInterestsFilter(value || undefined)
									}
								>
									<SelectTrigger>
														...
														<CardContent className="p-0 h-full flex flex-col">
															<div className="relative h-20 bg-gradient-to-r from-primary/20 to-primary/10">
																{member.banner_image && (
																	<img
																		src={
																			member.banner_image || "/placeholder.svg"
																		}
																		alt=""
...
export default function MembersPage() {
	return <ClientOnly><Members /></ClientOnly>
}
