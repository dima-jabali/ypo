...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (
      ...
      return NextResponse.json({ error: "`aws_bucket` and `aws_key` are required parameters" }, { status: 400 })
    }
    ...
    return NextResponse.json(
      { error: "Error fetching file from S3", details: error instanceof Error ? error.message : String(error) },
      { status: 500 },
    )
  }
}
