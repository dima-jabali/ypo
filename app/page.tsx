      ...
      ? members
          .filter((m) =>
            selectedLocations.some(
              (loc) => m.location?.includes(loc) || m.city?.includes(loc) || m.ypo_chapter?.includes(loc),
            ),
          )
          .slice(0, 5)
      : members.slice(0, 5)
    ...
    <main className="container mx-auto p-6 space-y-8">
      ...
      <section className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary/10 via-primary/5 to-background p-8 md:p-12 border border-primary/20">
        <div className="relative z-10">
          <Badge className="mb-4 bg-primary/20 text-primary border-primary/30">YPO for Enterprise</Badge>

          <h1 className="text-4xl md:text-5xl font-bold mb-4 text-balance">Your Unified Member Brain</h1>

          <p className="text-lg text-muted-foreground max-w-2xl mb-6 text-pretty">
            A single, intelligent layer powering world-class member connection, search, personalization, and future AI
            experiences for 38,000 global executives.
          </p>
      ...
      <section className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="cursor-pointer hover:border-primary transition-colors" onClick={() => router.push("/members")}>
        ...
        <Card className="cursor-pointer hover:border-primary transition-colors" onClick={() => router.push("/events")}>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
        ...
        <Card className="cursor-pointer hover:border-primary transition-colors" onClick={() => router.push("/content")}>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
      ...
      <Card className="bg-gradient-to-r from-accent/50 to-accent/20 border-accent">
        ...
        <CardContent>
          <div className="flex flex-wrap gap-2">
            {["New York", "Dubai", "Singapore", "London", "San Francisco", "Tokyo", "Mumbai", "Sydney"].map(
              (location) => (
                <Button
                  key={location}
                  ...
                  size="sm"
                  className={cn(selectedLocations.includes(location) && "bg-primary text-primary-foreground")}
                >
          ...
          {selectedLocations.length > 0 && (
            <Button variant="ghost" size="sm" onClick={() => setSelectedLocations([])} className="mt-3">
              Clear Selection
            </Button>
          )}
        </CardContent>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="cursor-pointer hover:border-primary transition-colors" onClick={() => router.push("/network")}>
          <CardHeader>
          ...
          <CardContent>
            <p className="text-sm text-muted-foreground">
              A living model that understands relationships across chapters, forums, networks, industries, and
              interests.
            </p>
          </CardContent>
        </Card>

        <Card className="cursor-pointer hover:border-primary transition-colors" onClick={() => router.push("/search")}>
          <CardHeader>
          ...
          <CardContent>
            <p className="text-sm text-muted-foreground">
              Multi-modal search across members, events, and content with intelligent recommendations and filters.
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-sm text-muted-foreground">
              Secure, governed AI tools via MCP for safe LLM access, predictions, and agentic workflows.
            </p>
          </CardContent>
        </Card>
      </section>
